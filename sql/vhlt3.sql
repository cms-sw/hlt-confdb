...
*/
  ...
  CURSOR cur_modules_from_paths IS
    SELECT UNIQUE
      v_paelements.id,
      v_mod2templ.id_templ,
      v_paelements.name
    FROM v_pathid2pae,v_paelements, v_pathid2conf, v_mod2templ
    WHERE v_pathid2conf.id_pathid=v_pathid2pae.id_pathid
    and v_pathid2pae.id_pae=v_paelements.id
    and v_paelements.paetype=1
    and v_mod2templ.id_pae=v_paelements.id
    and v_pathid2conf.id_confver = config_id order by v_paelements.name;
...
*/
  ...
  /* cursor for paths */
/*This is for h_ unique-ized tables
  CURSOR cur_paths IS
    SELECT
...
*/
  ...
  CURSOR cur_paths IS
    SELECT
      v_pathid2conf.id_pathid,
      v_paths.name,
      v_pathids.isEndPath,
      v_pathid2conf.ord
    FROM v_paths,v_pathid2conf,v_pathids
    WHERE v_pathids.id=v_pathid2conf.id_pathid
    and v_paths.id=v_pathids.id_path
    and v_pathid2conf.id_confver = config_id order by v_paths.name;
...
*/
  ...
  CURSOR cur_sequences IS
   SELECT UNIQUE
      ...
      v_paelements.name,
      0
    FROM v_paelements, v_pathid2conf,v_pathid2pae
    WHERE v_pathid2conf.id_pathid=v_pathid2pae.id_pathid 
    and v_pathid2pae.id_pae=v_paelements.id
...
*/
  ...
  CURSOR cur_path_sequence IS
     SELECT UNIQUE
      v_pathid2pae.id_pathid,
      v_paelements.id,
      0,
      v_pathid2pae.operator
    FROM v_pathid2pae,v_paelements, v_pathid2conf
    WHERE v_pathid2conf.id_pathid=v_pathid2pae.id_pathid
    and v_pathid2pae.id_pae=v_paelements.id
    and v_paelements.paetype=2
    and v_pathid2pae.lvl=0
...
*/
  ...
  CURSOR cur_path_outputmod IS
    SELECT
      ...
      v_pathid2outm.ord,
      v_pathid2outm.operator
    FROM v_pathid2outm,v_pathid2conf,v_streamids,v_pathids
    WHERE v_pathid2conf.id_pathid=v_pathids.id
    and v_streamids.id=v_pathid2outm.id_streamid
    and v_pathids.id=v_pathid2outm.id_pathId
    and v_pathid2conf.id_confver = config_id;
BEGIN
