...
def main(argv):
    ...
    myFixer = AddProblemModulesAndParametersMySQL(1,1)

    # Connect to DB, get module types
    connection = MySQLdb.connect(host=input_host, 
				 user=input_dbuser, passwd=input_dbpwd,
				 db=input_dbname )
    cursor = connection.cursor() 
    myFixer.ConfdbMySQLConnect(cursor, input_cmsswrel)
...
#	    print "SuperId is " + str(componentsuperid)
	    ...
	    if(len(varline) == 2):		
		print "\tComponent is " + varline[0] + ", parameter is " + vartype + " " + varname + " = " + str(varval) + " (" + str(vartracked) +") " + "sequence nb is " + str(thenewseq)

	    if(len(varline) == 3):
		print "\tComponent is " + varline[0] + ", parameter is " + " (in  (V)Pset " + varline[1] + ") " + vartype + " " + varname + " = " + str(varval) + " (" + str(vartracked) +") " + "sequence nb is " + str(thenewseq)
...
class AddProblemModulesAndParametersMySQL:
    ...
    def ConfdbMySQLConnect(self,thecursor,therelease):
