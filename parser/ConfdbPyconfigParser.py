...
class ConfdbPyconfigParser:
    ...
    def FindPythonConfigDefault(self,thecomponent,thedirectory):
        ...
        self.SearchInPythonDirectories(thecomponent,thedirectory)

    def SearchInPythonDirectories(self,thecomponent,thedirectory):
        # Look at what cfi_py configs are available
        if(thedirectory.find('data/') != -1 and thedirectory.find('python/') == -1):
        ...
        if(os.path.isdir(thedirectory)):
            ...
            for cfipyfile in cfipyfiles:
                if(cfipyfile.endswith('_cfi.py')):
                    self.SearchInPythonConfigFile(cfipyfile,thedirectory,thecomponent)
                if(os.path.isdir(thedirectory + "/" + cfipyfile)):
                    self.SearchInPythonDirectories(thecomponent,thedirectory + "/" + cfipyfile + "/")

    def SearchInPythonConfigFile(self,cfipyfile,thedirectory,thecomponent):
        thefilecomponent = cfipyfile.split('.py')[0]
                    
        thebasefilecomponent = thefilecomponent.split('_cfi')[0]        

        # Construct the py-cfi to import
        thesubsystempackage = thedirectory
        if(thesubsystempackage.find('data/') != -1 and thesubsystempackage.find('python/') == -1):
            thesubsystempackage = thedirectory.split('src/')[1].split('/data/')[0].lstrip().rstrip()
        if(thesubsystempackage.find('python/') != -1):
            thesubsystempackage = thedirectory.split('src/')[1].split('/python/')[0].lstrip().rstrip() 

        thesubsystem = thesubsystempackage.split('/')[0]
        thepackage = thesubsystempackage.split('/')[1]

        subdirs = ''
        if(thedirectory.endswith('python/')):
            subdirs = ''
        else:
            tmpsubdirs = thedirectory.split('python//')[1]
            subdirs = tmpsubdirs.replace('/','.')

        importcommand = "import " + thesubsystem + "." + thepackage
        if(subdirs == ''):
            importcommand = importcommand + "." + thefilecomponent
        else:
            importcommand = importcommand + "." + subdirs + thefilecomponent
        ...
        try:
            ...
            theextend = "process.extend(" + thesubsystem + "." + thepackage
            if(subdirs == ''):
                theextend = theextend + "." + thefilecomponent + ")"
            else:
                theextend = theextend + "." + subdirs + thefilecomponent + ")"
            if(self.verbose > 2):
                print "\t\t" + theextend
            eval(theextend)
