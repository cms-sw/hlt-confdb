...
public class DatasetComparison extends Comparison {
	...
	public int result() {
		if (oldDataset == null && newDataset != null)
			return RESULT_ADDED;
		else if (oldDataset != null && newDataset == null)
			return RESULT_REMOVED;
		else if (comparisonCount() == 0 && oldDataset.name().equals(newDataset.name()))
	...
	public String toString() {
		return (newDataset == null) ? "Dataset " + oldDataset.name() + " " + resultAsString()
				: "Dataset " + newDataset.name() + " " + resultAsString();
	}

	/** html representation of the comparison */
	public String toHtml() {
		return (newDataset == null) ? "<html>Dataset <b>" + oldDataset.name() + "</b></html>"
