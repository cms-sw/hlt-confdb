...
public class ConfigurationTreeModel extends AbstractTreeModel {
	...
	public StringBuffer essourcesNode() {
		return essourcesNode;
	}

	/** get the ESModules root node */
	public StringBuffer esmodulesNode() {
		return esmodulesNode;
	}

	/** get the services root node */
	public StringBuffer servicesNode() {
		return servicesNode;
	}

	/** get the paths root node */
	public StringBuffer pathsNode() {
		return pathsNode;
	}
	...
	public StringBuffer outputsNode() {
		return outputsNode;
	}

	/** get the contents root node */
	public StringBuffer contentsNode() {
		return contentsNode;
	}

	/** get the streams root node */
	public StringBuffer streamsNode() {
		return streamsNode;
	}

	/** get the datasets root node */
	public StringBuffer datasetsNode() {
		return datasetsNode;
	}
	...
	public boolean setContentMode(String contentMode) {
		if (contentMode.equals("streams") || contentMode.equals("paths") || contentMode.equals("datasets")) {
			this.contentMode = contentMode;
			return true;
		}
		System.err.println("ConfigurationTreeModel.setContentMode() ERROR: " + "invalid mode '" + contentMode + "'");
		return false;
	}
	...
	public boolean setStreamMode(String streamMode) {
		...
		System.err.println("ConfigurationTreeModel.setStreamMode() ERROR: " + "invalid mode '" + streamMode + "'");
		return false;
	}
	...
	public void updateLevel1Nodes() {
		...
		if (unassignedCount == 0)
			pathsNode.append(" <font color=#00ff00>[").append(unassignedCount).append("]</font>");
		else
			pathsNode.append(" <font color=#ff0000>[").append(unassignedCount).append("]</font>");
	...
	public int getChildCount(Object node) {
		if (node.equals(config)) {
			...
			if (node.equals(essourcesNode))
				return config.essourceCount();
			if (node.equals(esmodulesNode))
				return config.esmoduleCount();
			if (node.equals(servicesNode))
				return config.serviceCount();
	...
	public Object getChild(Object parent, int i) {
		if (parent.equals(config)) {
			...
			if (parent.equals(essourcesNode))
				return config.essource(i);
			if (parent.equals(esmodulesNode))
				return config.esmodule(i);
			if (parent.equals(servicesNode))
				return config.service(i);
			...
			if (streamMode.equals("datasets"))
				...
				return (i < stream.datasetCount()) ? new ConfigurationTreeNode(stream, stream.dataset(i))
			...
			if (treeNode.object() instanceof PrimaryDataset) {
				...
				Stream stream = (Stream) treeNode.parent();
				return new ConfigurationTreeNode(treeNode, stream.listOfUnassignedPaths().get(i));
			}
		}
	...
	public int getIndexOfChild(Object parent, Object child) {
		if (parent.equals(config)) {
			...
			if (treeNode.object() instanceof PrimaryDataset) {
				 ...
				 * (ConfigurationTreeNode)treeNode.parent(); stream =
				 * (Stream)parentTreeNode.parent(); Path path = (Path)child; int index =
				 * stream.listOfUnassignedPaths().indexOf(path); return index;
				 */
			}
			// else if (treeNode.object() instanceof Stream
		}
