...
public class ImportTreeMouseListener extends MouseAdapter {
	...
	private void maybeShowPopup(MouseEvent e) {
		...
		TreePath tp = importTree.getPathForLocation(e.getX(), e.getY());
		if (tp == null)
			return;
			...
			if (tm.getChildCount(node) != 0) {
				...
				if (child instanceof ReferenceContainer) {
					...
					if (child instanceof ServiceInstance)
						item = new JMenuItem("Import all Services");
					else if (child instanceof ESModuleInstance)
						item = new JMenuItem("Import all ESModules");
					else if (child instanceof ESSourceInstance)
						item = new JMenuItem("Import all ESSources");
...
class AddContainerListener implements ActionListener {
	...
	// construction
	public AddContainerListener(JTree targetTree, ReferenceContainer container) {
...
class AddDeepContainerListener implements ActionListener {
	...
	public AddDeepContainerListener(JTree targetTree, Configuration sourceConfig, ReferenceContainer container) {
		this.targetTree = targetTree;
		this.container = container;
