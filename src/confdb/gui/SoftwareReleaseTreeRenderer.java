...
public class SoftwareReleaseTreeRenderer extends DefaultTreeCellRenderer {
	...
	public Icon prepareIcon() {
		...
		if (node instanceof SoftwarePackage)
			return packageIcon;
	...
	public String prepareText() {
		...
		if (node instanceof SoftwareSubsystem) {
			...
			result = (count > 0) ? "<html><b>" + s.name() + " (" + count + "/" + max + ")</b></html>"
			...
			result = (count > 0) ? "<html><b>" + p.cvsTag() + " " + p.name() + " (" + count + "/" + max + ")</b></html>"
					: "<html>" + p.cvsTag() + " " + p.name() + " (" + count + "/" + max + ")</html>";
		} else if (node instanceof Template) {
			Template t = (Template) node;
			int count = t.instanceCount();
			result = (count > 0) ? "<html><b>" + t.name() + " [" + t.type() + "] (" + count + ")</b></html>"
