...
public class NewConfigurationDialog extends JDialog implements ActionListener, PropertyChangeListener {
	...
	public NewConfigurationDialog(JFrame frame, ConfDB database) {
		...
		Object[] components = { new JLabel("Name:"), textFieldName, new JLabel("Process:"), textFieldProcess,
				new JLabel("Release:"), comboBoxReleaseTag };
		Object[] options = { ok, cancel };
		optionPane = new JOptionPane(components, JOptionPane.QUESTION_MESSAGE, JOptionPane.YES_NO_OPTION, null, options,
				options[0]);
	...
	public void propertyChange(PropertyChangeEvent e) {
		...
		if (isVisible() && e.getSource() == optionPane && (JOptionPane.VALUE_PROPERTY.equals(propertyName)
			...
			if (ok.equals(value)) {
				if (textFieldName.getText().length() > 0 && textFieldProcess.getText().length() > 0
					...
					System.out.println("You must specify name&release for your " + "new configuration!");
					return;
				}
			} else {
				name = null;
