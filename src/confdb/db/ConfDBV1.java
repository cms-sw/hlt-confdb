...
public class ConfDBV1 {
	...
	public synchronized void reconnect() throws DatabaseException {
		...
		do {
			...
			if (retryCount != 5)
				System.err.println("[ConfDBV1::reconnect]" + ft.format(dNow) + " Trying to connect... attemp ("
						+ (5 - retryCount) + ")");
			...
			try {
				...
				if (rs != null) {
					try {
						...
						System.err.println("[ConfDBV1::reconnect()]" + ft.format(dNow) + " Error closing ResultSet! "
								+ sqlEx.getMessage());
					}
				}
		...
		if (!transactionCompleted)
			throw new DatabaseException("ConfDBV1::reconnect(): Unable to connect!");
		else if (retryCount < 5)
			System.out.println("[ConfDBV1::reconnect] connection reestablished!");
	...
	public synchronized Directory loadConfigurationTree() throws DatabaseException {
		...
		try {
			...
			while (rs.next()) {
				...
				if (dir == null) {
					String errMsg = "ConfDBV1::loadConfigurationTree(): can't find directory " + "for parentDirId="
							+ parentDirId + ".";
			...
			while (rs.next()) {
				...
				ConfigInfo configInfo = configHashMap.get(configPathAndName);
				if (configInfo == null) {
					String errMsg = "ConfDBV1::loadConfigurationTree(): can't find locked " + "configuration '"
							+ configPathAndName + "'.";
					throw new DatabaseException(errMsg);
				}
			...
			String errMsg = "ConfDBV1::loadConfigurationTree() failed: " + e.getMessage();
			throw new DatabaseException(errMsg, e);
		} finally {
			dbConnector.release(rs);
		}
	...
	public synchronized Template loadTemplate(String releaseTag, String templateName) throws DatabaseException {
		...
		try {
			...
			String errMsg = "ConfDBV1::loadTemplate(releaseTag=" + releaseTag + ",templateName=" + templateName
					+ ") failed: " + e.getMessage();
			throw new DatabaseException(errMsg, e);
		}
	...
	public synchronized void loadSoftwareRelease(int releaseId, SoftwareRelease release) throws DatabaseException {
		...
		try {
			...
			String errMsg = "ConfDBV1::loadSoftwareRelease(releaseId=" + releaseId + ",release) failed: "
					+ e.getMessage();
			throw new DatabaseException(errMsg, e);
		}
	...
	public synchronized void loadPartialSoftwareRelease(int configId, SoftwareRelease release)
		...
		try {
			...
			String errMsg = "ConfDBV1::loadPartialSoftwareRelease(configId=" + configId + ",release) failed: "
					+ e.getMessage();
			throw new DatabaseException(errMsg, e);
		}
	...
	private void loadTemplates(CallableStatement cs, SoftwareRelease release) throws DatabaseException {
		...
		try {
			...
			String errMsg = "ConfDBV1::loadTemplates() failed: " + e.getMessage();
			throw new DatabaseException(errMsg, e);
		} finally {
	...
	private void loadConfiguration(Configuration config) throws DatabaseException {
		...
		try {
			...
			// This is to fix the operator field bug for modules inside sequences. bug
			// #91797
			...
			while (rsSequenceEntries.next()) {
				...
				try {
					...
					System.out.println(
							"SQLException catched at confDb.java::loadConfiguration()   sequence = " + sequence.name());
				}

				if (index != sequenceNb)
					System.err.println(
							"ERROR in sequence " + sequence.name() + ": index=" + index + " sequenceNb=" + sequenceNb);
			...
			/*
			 * Iterator<Path> pathIt = config.pathIterator(); while(pathIt.hasNext()){ Path
			 * path = pathIt.next(); int databaseId = pathToId.get(path);
			 * path.setDatabaseId(databaseId); }
			 */

		} catch (SQLException e) {
			String errMsg = "ConfDBV1::loadConfiguration(Configuration config) failed " + "(configId=" + configId
					+ "): " + e.getMessage();
			throw new DatabaseException(errMsg, e);
		} finally {
	...
	public synchronized boolean checkExtraPathFields() throws DatabaseException {
		...
		try {
			...
			String errMsg = "ConfDBV1::checkExtraPathFields failed: " + e.getMessage();
			throw new DatabaseException(errMsg, e);
		} finally {
			dbConnector.release(rs);
		}
	...
	public synchronized boolean checkOperatorFieldForSequences() throws DatabaseException {
		...
		try {
			...
			String errMsg = "ConfDBV1::checkOperatorFieldForSequences failed: " + e.getMessage();
			throw new DatabaseException(errMsg, e);
		} finally {
			dbConnector.release(rs);
		}
	...
	public synchronized void insertDirectory(Directory dir) throws DatabaseException {
		...
		try {
			...
			String errMsg = "ConfDBV1::insertDirectory(Directory dir) failed " + "(parentDirId="
					+ dir.parentDir().dbId() + ",name=" + dir.name() + "): " + e.getMessage();
			throw new DatabaseException(errMsg, e);
		} finally {
			dbConnector.release(rs);
		}
	...
	public synchronized void removeDirectory(Directory dir) throws DatabaseException {
		try {
			...
			String errMsg = "ConfDBV1::removeDirectory(Directory dir) failed " + "(name=" + dir.name() + "): "
					+ e.getMessage();
			throw new DatabaseException(errMsg, e);
		}
	}
	...
	public synchronized void insertConfiguration(Configuration config, String creator, String processName,
		...
		try {
			...
			String errMsg = "ConfDBV1::insertConfiguration(config=" + config.dbId() + ",creator=" + creator
	...
	public synchronized void lockConfiguration(Configuration config, String userName) throws DatabaseException {
		...
		if (config.isLocked()) {
			String errMsg = "ConfDBV1::lockConfiguration(): Can't lock " + config.toString()
		...
		try {
			...
			String errMsg = "ConfDBV1::lockConfiguration(" + config.toString() + ") failed: " + e.getMessage();
			throw new DatabaseException(errMsg, e);
		}
	}
	...
	public synchronized void unlockConfiguration(Configuration config) throws DatabaseException {
		...
		try {
			...
			String errMsg = " ConfDBV1::unlockConfiguration(" + config.toString() + " failed: " + e.getMessage();
			throw new DatabaseException(errMsg, e);
		}
	}
	...
	private void insertGlobalPSets(int configId, Configuration config) throws DatabaseException {
		for (int sequenceNb = 0; sequenceNb < config.psetCount(); sequenceNb++) {
			...
			try {
				...
				String errMsg = "ConfDBV1::insertGlobalPSets(configId=" + configId + ") failed: " + e.getMessage();
				throw new DatabaseException(errMsg, e);
			}
		}
		config.psets().setDatabaseId(1);
	...
	private void insertEDSources(int configId, Configuration config) throws DatabaseException {
		for (int sequenceNb = 0; sequenceNb < config.edsourceCount(); sequenceNb++) {
			...
			if (edsourceId <= 0) {
				...
				try {
					...
					String errMsg = "ConfDBV1::insertEDSources(configID=" + configId + ") failed (edsourceId="
		...
		try {
			...
			String errMsg = "ConfDBV1::insertEDSources(configId=" + configId + ") failed " + "(batch insert):"
					+ e.getMessage();
			throw new DatabaseException(errMsg, e);
		}
	}
	...
	private void insertESSources(int configId, Configuration config) throws DatabaseException {
		for (int sequenceNb = 0; sequenceNb < config.essourceCount(); sequenceNb++) {
			...
			if (essourceId <= 0) {
				...
				try {
					...
					String errMsg = "ConfDBV1::insertESSources(configID=" + configId + ") failed " + "(essourceId="
		...
		try {
			...
			String errMsg = "ConfDBV1::insertESSources(configId=" + configId + ") failed " + "(batch insert):"
					+ e.getMessage();
			throw new DatabaseException(errMsg, e);
		}
	}
	...
	private void insertESModules(int configId, Configuration config) throws DatabaseException {
		for (int sequenceNb = 0; sequenceNb < config.esmoduleCount(); sequenceNb++) {
			...
			if (esmoduleId <= 0) {
				...
				try {
					...
					String errMsg = "ConfDBV1::insertESModules(configID=" + configId + ") failed " + "(esmoduleId="
		...
		try {
			...
			String errMsg = "ConfDBV1::insertESModule(configId=" + configId + ") failed " + "(batch insert):"
					+ e.getMessage();
			throw new DatabaseException(errMsg, e);
		}
	}
	...
	private void insertServices(int configId, Configuration config) throws DatabaseException {
		for (int sequenceNb = 0; sequenceNb < config.serviceCount(); sequenceNb++) {
			...
			if (serviceId <= 0) {
				...
				try {
					...
					String errMsg = "ConfDBV1::insertServices(configID=" + configId + ") failed " + "(serviceId="
			...
			try {
				...
				String errMsg = "ConfDBV1::insertServices(configID=" + configId + ") failed " + "(serviceId="
						+ serviceId + ", sequenceNb=" + sequenceNb + "): " + e.getMessage();
				throw new DatabaseException(errMsg, e);
			}
		}
		...
		try {
			...
			String errMsg = "ConfDBV1::insertService(configId=" + configId + ") failed " + "(batch insert):"
					+ e.getMessage();
			throw new DatabaseException(errMsg, e);
		}
	}
	...
	private HashMap<String, Integer> insertPaths(int configId, Configuration config) throws DatabaseException {
		...
		try {
			...
			String errMsg = "ConfDBV1::insertPaths(configId=" + configId + ") failed: " + e.getMessage();
			throw new DatabaseException(errMsg, e);
		} finally {
			dbConnector.release(rs);
		}
		...
		try {
			...
			String errMsg = "ConfDBV1::insertPaths(configId=" + configId + ") failed (batch insert): " + e.getMessage();
			throw new DatabaseException(errMsg, e);
		}
	...
	private HashMap<String, Integer> insertSequences(int configId, Configuration config) throws DatabaseException {
		...
		try {
			...
			String errMsg = "ConfDBV1::insertSequences(configId=" + configId + ") failed: " + e.getMessage();
			throw new DatabaseException(errMsg, e);
		} finally {
			dbConnector.release(rs);
		}
		...
		try {
			...
			String errMsg = "ConfDBV1::insertSequences(configId=" + configId + ") failed " + "(batch insert): "
					+ e.getMessage();
			throw new DatabaseException(errMsg, e);
		}
	...
	private HashMap<String, Integer> insertModules(Configuration config) throws DatabaseException {
		...
		for (int i = 0; i < config.moduleCount(); i++) {
			...
			if (moduleId > 0) {
				...
				try {
					...
					String errMsg = "ConfDBV1::insertModules(config=" + config.toString() + " failed (moduleId="
		...
		try {
			...
			String errMsg = "ConfDBV1::insertModules(configId=" + config.toString() + ") failed " + "(batch insert): "
					+ e.getMessage();
			throw new DatabaseException(errMsg, e);
		}
	...
	private void insertReferences(Configuration config, HashMap<String, Integer> pathHashMap,
			HashMap<String, Integer> sequenceHashMap, HashMap<String, Integer> moduleHashMap,
				...
				for (int sequenceNb = 0; sequenceNb < path.entryCount(); sequenceNb++) {
					...
					if (r instanceof PathReference) {
						...
						try {
							...
							String errMsg = "ConfDBV1::insertReferences(config=" + config.toString()
									+ ") failed (pathId=" + pathId + ",childPathId=" + childPathId + ",sequenceNb="
						...
						try {
							...
							String errMsg = "ConfDBV1::insertReferences(config=" + config.toString()
									+ ") failed (pathId=" + pathId + ",sequenceId=" + sequenceId + ",sequenceNb="
									+ sequenceNb + "): " + e.getMessage();
							throw new DatabaseException(errMsg, e);
						}
						...
						try {
							...
							String errMsg = "ConfDBV1::insertReferences(config=" + config.toString()
									+ ") failed (pathId=" + pathId + ",moduleId=" + moduleId + ",sequenceNb="
									+ sequenceNb + "): " + e.getMessage();
							throw new DatabaseException(errMsg, e);
						}
						...
						try {
							...
							String errMsg = "ConfDBV1::insertReferences(config=" + config.toString()
									+ ") failed (pathId=" + pathId + ",moduleId=" + outputModuleId + ",sequenceNb="
									+ sequenceNb + "): " + e.getMessage();
							throw new DatabaseException(errMsg, e);
						}
					}
				...
				for (int sequenceNb = 0; sequenceNb < sequence.entryCount(); sequenceNb++) {
					...
					if (r instanceof SequenceReference) {
						...
						try {
							...
							e.printStackTrace();
							String errMsg = "ConfDBV1::insertReferences(config=" + config.toString()
									+ ") failed (sequenceId=" + sequenceId + " (" + sequence.name()
									+ "), childSequenceId=" + childSequenceId + " (" + r.name() + ")" + ",sequenceNb="
									+ sequenceNb + "): " + e.getMessage();
							throw new DatabaseException(errMsg, e);
						}
					} else if (r instanceof ModuleReference) {
						...
						try {
							...
							String errMsg = "ConfDBV1::insertReferences(config=" + config.toString()
									+ ") failed (sequenceId=" + sequenceId + ",moduleId=" + moduleId + ",sequenceNb="
									+ sequenceNb + "): " + e.getMessage();
							throw new DatabaseException(errMsg, e);
						}
					} else if (r instanceof OutputModuleReference) {
						...
						try {
							...
							String errMsg = "ConfDBV1::insertReferences(config=" + config.toString()
									+ ") failed (sequenceId=" + sequenceId + ",outputmoduleId=" + outputModuleId
		...
		try {
			...
			String errMsg = "ConfDBV1::insertReferences(config=" + config.toString() + ") failed " + "(batch insert): "
					+ e.getMessage();
			throw new DatabaseException(errMsg, e);
		}
	}
	...
	private HashMap<String, Integer> insertEventContents(int configId, Configuration config) throws DatabaseException {
		...
		while (itC.hasNext()) {
			...
			try {
				...
				String errMsg = "ConfDBV1::Event Content(config=" + config.toString() + ") failed " + "(batch insert): "
						+ e.getMessage();
				throw new DatabaseException(errMsg, e);
			}
			eventContent.setDatabaseId(eventContentId);
		...
		while (itC.hasNext()) {
			...
			try {
				...
				String errMsg = "ConfDBV1::Event Content Config Association (config=" + config.toString()
	...
	private void insertEventContentStatements(int configId, Configuration config,
		...
		for (int i = 0; i < config.contentCount(); i++) {
			...
			for (int j = 0; j < eventContent.commandCount(); j++) {
				...
				try {
					...
					String errMsg = "ConfDBV1::StatementID Update(config=" + config.toString()
							+ ") failed (batch insert): " + e.getMessage();
					throw new DatabaseException(errMsg, e);
				}
		...
		try {
			...
			String errMsg = "ConfDBV1::StatementID Update(config=" + config.toString() + ") failed "
	...
	private HashMap<String, Integer> insertStreams(int configId, Configuration config) throws DatabaseException {
		...
		while (itS.hasNext()) {
			...
			try {
				...
				String errMsg = "ConfDBV1::Streams(config=" + config.toString() + ") failed " + "(batch insert): "
						+ e.getMessage();
				throw new DatabaseException(errMsg, e);
			}
	...
	private HashMap<String, Integer> insertPrimaryDatasets(int configId, Configuration config)
		...
		while (itP.hasNext()) {
			...
			try {
				...
				String errMsg = "ConfDBV1::Primary Dataset (config=" + config.toString() + ") failed "
						+ "(batch insert): " + e.getMessage();
				throw new DatabaseException(errMsg, e);
			}
		}
		...
		for (int i = 0; i < config.contentCount(); i++) {
			...
			for (int j = 0; j < eventContent.streamCount(); j++) {
				...
				try {
					...
					String errMsg = "ConfDBV1::Event Content(config=" + config.toString() + ") failed "
							+ "(batch insert): " + e.getMessage();
					throw new DatabaseException(errMsg, e);
				}
			}
	...
	private void insertStreamDatasetAssoc(HashMap<String, Integer> streamHashMap,
			HashMap<String, Integer> primaryDatasetHashMap, Configuration config) throws DatabaseException {
		for (int i = 0; i < config.streamCount(); i++) {
			...
			for (int j = 0; j < stream.datasetCount(); j++) {
				...
				try {
					...
					String errMsg = "ConfDBV1::Stream Primary dataset association(config=" + config.toString()
							+ ") failed " + "(batch insert): " + e.getMessage();
					throw new DatabaseException(errMsg, e);
				}
			}
		...
		try {
			...
			String errMsg = "ConfDBV1::Stream Primary dataset association(config=" + config.toString() + ") failed "
					+ "(batch insert): " + e.getMessage();
			throw new DatabaseException(errMsg, e);
		}
	}
	 ...
	 * errMsg = "ConfDBV1::Event Content(config="+config.toString()+") failed "+
	...
	private void insertInstanceParameters(int superId, Instance instance) throws DatabaseException {
	...
	public synchronized String[] getConfigNamesByRelease(int releaseId) throws DatabaseException {
		...
		try {
			...
			String errMsg = "ConfDBV1::getConfigNamesByRelease() failed: " + e.getMessage();
			throw new DatabaseException(errMsg, e);
		} finally {
			dbConnector.release(rs);
		}
	...
	public synchronized int getDirectoryId(String directoryName) throws DatabaseException {
		...
		try {
			...
			String errMsg = "ConfDBV1::getDirectoryId(directoryName=" + directoryName + ") failed: " + e.getMessage();
			throw new DatabaseException(errMsg, e);
		} finally {
			dbConnector.release(rs);
		}
	...
	public synchronized HashMap<Integer, Directory> getDirectoryHashMap() throws DatabaseException {
		...
		try {
			...
			String errMsg = "ConfDBV1::getDirectoryHashMap() failed: " + e.getMessage();
			throw new DatabaseException(errMsg, e);
		} finally {
			dbConnector.release(rs);
		}
	...
	public synchronized int getConfigId(String fullConfigName) throws DatabaseException {
		...
		if (index < 0) {
			String errMsg = "ConfDBV1::getConfigId(fullConfigName=" + fullConfigName + ") failed (invalid name).";
			throw new DatabaseException(errMsg);
		}
		...
		try {
			...
			String errMsg = "ConfDBV1::getConfigId(fullConfigName=" + fullConfigName + ") failed (dirName=" + dirName
	...
	public synchronized ConfigInfo getConfigInfo(int configId) throws DatabaseException {
		...
		if (result == null) {
			String errMsg = "ConfDBV1::getConfigInfo(configId=" + configId + ") failed.";
			throw new DatabaseException(errMsg);
		}
		return result;
	}

	/** get available cmssw and architectures */
	...
	/** insert all instance parameters */
	private void insertTemplateParameters(int superId, Template template) throws DatabaseException {
	...
	public synchronized void removeSoftwareRelease(int releaseId) throws DatabaseException {
		if (getConfigNamesByRelease(releaseId).length > 0) {
			System.err.println("ConfDBV1::removeSoftwareRelease ERROR: " + "Can't remove release with associated "
					+ "configurations!)");
			return;
		}
	...
	private void prepareStatements() throws DatabaseException {
		...
		try {
			...
			psSelectModuleTypes = dbConnector.getConnection()
					.prepareStatement("SELECT" + " ModuleTypes.typeId," + " ModuleTypes.type " + "FROM ModuleTypes");
			preparedStatements.add(psSelectModuleTypes);

			psSelectParameterTypes = dbConnector.getConnection().prepareStatement(
					"SELECT" + " ParameterTypes.paramTypeId," + " ParameterTypes.paramType " + "FROM ParameterTypes");
			preparedStatements.add(psSelectParameterTypes);

			psSelectDirectories = dbConnector.getConnection()
					.prepareStatement("SELECT" + " Directories.dirId," + " Directories.parentDirId,"
							+ " Directories.dirName," + " Directories.created " + "FROM Directories "
							+ "ORDER BY Directories.dirName ASC");
			psSelectDirectories.setFetchSize(512);
			preparedStatements.add(psSelectDirectories);

			psSelectConfigurations = dbConnector.getConnection()
					.prepareStatement("SELECT" + " Configurations.configId," + " Configurations.parentDirId,"
							+ " Configurations.config," + " Configurations.version," + " Configurations.created,"
							+ " Configurations.creator," + " SoftwareReleases.releaseTag,"
							+ " Configurations.processName," + " Configurations.description " + "FROM Configurations "
							+ "JOIN SoftwareReleases " + "ON SoftwareReleases.releaseId = Configurations.releaseId "
			...
			psSelectLockedConfigurations = dbConnector.getConnection()
					.prepareStatement("SELECT" + " Directories.dirName," + " LockedConfigurations.config,"
							+ " LockedConfigurations.userName " + "FROM LockedConfigurations " + "JOIN Directories "
			...
			psSelectUsersForLockedConfigs = dbConnector.getConnection()
					.prepareStatement("SELECT" + " LockedConfigurations.userName " + "FROM LockedConfigurations");
			preparedStatements.add(psSelectUsersForLockedConfigs);

			psSelectSwArchNames = dbConnector.getConnection().prepareStatement(
					"select unique(SW_ARCH) HLT_SW_ARCH from ( select CMSSW_REL || ' ' || ARCH as SW_ARCH from CMS_HLT_GDR.HLT_CMSSW_ARCH where CMSSW_REL is not null) order by HLT_SW_ARCH desc");
			...
			psSelectConfigNames = dbConnector.getConnection()
					.prepareStatement("SELECT DISTINCT" + " Directories.dirName," + " Configurations.config "
							+ "FROM Configurations " + "JOIN Directories "
			...
			psSelectConfigNamesByRelease = dbConnector.getConnection()
					.prepareStatement("SELECT DISTINCT" + " Directories.dirName," + " Configurations.config, "
							+ " Configurations.version " + "FROM Configurations " + "JOIN Directories "
							+ "ON Configurations.parentDirId = Directories.dirId "
			...
			psSelectDirectoryId = dbConnector.getConnection().prepareStatement(
					"SELECT" + " Directories.dirId " + "FROM Directories " + "WHERE Directories.dirName = ?");
			preparedStatements.add(psSelectDirectoryId);

			psSelectConfigurationId = dbConnector.getConnection()
					.prepareStatement("SELECT" + " Configurations.configId " + "FROM Configurations "
							+ "JOIN Directories " + "ON Directories.dirId=Configurations.parentDirId "
							+ "WHERE Directories.dirName = ? AND" + " Configurations.config = ? AND"
			...
			psSelectConfigurationIdLatest = dbConnector.getConnection()
					.prepareStatement("SELECT" + " Configurations.configId," + " Configurations.version "
							+ "FROM Configurations " + "JOIN Directories "
							+ "ON Directories.dirId=Configurations.parentDirId " + "WHERE Directories.dirName = ? AND"
							+ " Configurations.config = ? " + "ORDER BY Configurations.version DESC");
			preparedStatements.add(psSelectConfigurationIdLatest);

			psSelectConfigurationCreated = dbConnector.getConnection().prepareStatement("SELECT"
					+ " Configurations.created " + "FROM Configurations " + "WHERE Configurations.configId = ?");
			preparedStatements.add(psSelectConfigurationCreated);

			psSelectReleaseTags = dbConnector.getConnection()
					.prepareStatement("SELECT" + " SoftwareReleases.releaseId," + " SoftwareReleases.releaseTag "
							+ "FROM SoftwareReleases " + "ORDER BY SoftwareReleases.releaseId DESC");
			...
			psSelectReleaseTagsSorted = dbConnector.getConnection()
					.prepareStatement("SELECT" + " SoftwareReleases.releaseId," + " SoftwareReleases.releaseTag "
							+ "FROM SoftwareReleases " + "ORDER BY SoftwareReleases.releaseTag ASC");
			psSelectReleaseTagsSorted.setFetchSize(32);
			preparedStatements.add(psSelectReleaseTagsSorted);

			psSelectReleaseId = dbConnector.getConnection().prepareStatement("SELECT" + " SoftwareReleases.releaseId "
					+ "FROM SoftwareReleases " + "WHERE SoftwareReleases.releaseTag = ?");

			psSelectReleaseTag = dbConnector.getConnection().prepareStatement("SELECT" + " SoftwareReleases.releaseTag "
					+ "FROM SoftwareReleases " + "WHERE SoftwareReleases.releaseId = ?");
			preparedStatements.add(psSelectReleaseTag);

			psSelectReleaseTagForConfig = dbConnector.getConnection()
					.prepareStatement("SELECT" + " SoftwareReleases.releaseTag " + "FROM SoftwareReleases "
							+ "JOIN Configurations " + "ON Configurations.releaseId = SoftwareReleases.releaseId "
							+ "WHERE Configurations.configId = ?");
			preparedStatements.add(psSelectReleaseTagForConfig);

			psSelectSoftwareSubsystems = dbConnector.getConnection().prepareStatement("SELECT"
					+ " SoftwareSubsystems.subsysId," + " SoftwareSubsystems.name " + "FROM SoftwareSubsystems");
			...
			psSelectSoftwarePackages = dbConnector.getConnection()
					.prepareStatement("SELECT" + " SoftwarePackages.packageId," + " SoftwarePackages.subsysId,"
							+ " SoftwarePackages.name " + "FROM SoftwarePackages");
			...
			psSelectEDSourceTemplate = dbConnector.getConnection()
					.prepareStatement("SELECT" + " EDSourceTemplates.superId," + " EDSourceTemplates.name,"
							+ " EDSourceTemplates.cvstag " + "FROM EDSourceTemplates "
			...
			psSelectESSourceTemplate = dbConnector.getConnection()
					.prepareStatement("SELECT" + " ESSourceTemplates.superId," + " ESSourceTemplates.name,"
							+ " ESSourceTemplates.cvstag " + "FROM ESSourceTemplates " + "WHERE name=? AND cvstag=?");
			preparedStatements.add(psSelectESSourceTemplate);

			psSelectESModuleTemplate = dbConnector.getConnection()
					.prepareStatement("SELECT" + " ESModuleTemplates.superId," + " ESModuleTemplates.name,"
							+ " ESModuleTemplates.cvstag " + "FROM ESModuleTemplates " + "WHERE name=? AND cvstag=?");
			preparedStatements.add(psSelectESModuleTemplate);

			psSelectServiceTemplate = dbConnector.getConnection()
					.prepareStatement("SELECT" + " ServiceTemplates.superId," + " ServiceTemplates.name,"
							+ " ServiceTemplates.cvstag " + "FROM ServiceTemplates " + "WHERE name=? AND cvstag=?");
			preparedStatements.add(psSelectServiceTemplate);

			psSelectModuleTemplate = dbConnector.getConnection()
					.prepareStatement("SELECT" + " ModuleTemplates.superId," + " ModuleTemplates.typeId,"
							+ " ModuleTemplates.name," + " ModuleTemplates.cvstag " + "FROM ModuleTemplates "
							+ "WHERE name=? AND cvstag=?");
			preparedStatements.add(psSelectModuleTemplate);

			psSelectStreams = dbConnector.getConnection().prepareStatement("SELECT" + " Streams.streamId,"
					+ " Streams.streamLabel " + "FROM Streams " + "ORDER BY Streams.streamLabel ASC");

			psSelectPrimaryDatasets = dbConnector.getConnection()
					.prepareStatement("SELECT" + " PrimaryDatasets.datasetId," + " PrimaryDatasets.datasetLabel "
							+ "FROM PrimaryDatasets " + "ORDER BY PrimaryDatasets.datasetLabel ASC");

			psSelectPrimaryDatasetEntries = dbConnector.getConnection().prepareStatement("SELECT"
					+ " PrimaryDatasetPathAssoc.datasetId," + " PrimaryDatasets.datasetLabel,"
					+ " PrimaryDatasetPathAssoc.pathId " + "FROM PrimaryDatasetPathAssoc " + "JOIN PrimaryDatasets "
					+ "ON PrimaryDatasets.datasetId=PrimaryDatasetPathAssoc.datasetId " + "JOIN ConfigurationPathAssoc "
			...
			psSelectPSetsForConfig = dbConnector.getConnection()
					.prepareStatement("SELECT" + " ParameterSets.superId " + "FROM ParameterSets "
							+ "JOIN ConfigurationParamSetAssoc " + "ON ConfigurationParamSetAssoc.psetId="
							+ "ParameterSets.superId " + "WHERE ConfigurationParamSetAssoc.configId=?");
			preparedStatements.add(psSelectPSetsForConfig);

			psSelectContentForConfig = dbConnector.getConnection()
					.prepareStatement("SELECT " + "EventContents.eventContentId " + "FROM	EventContents "
							+ "JOIN	ConfigurationContentAssoc " + "ON		ConfigurationContentAssoc.eventContentId "
							+ "	= 	EventContents.eventContentId " + "WHERE	ConfigurationContentAssoc.configId = ?");
			preparedStatements.add(psSelectContentForConfig);

			psSelectEDSourcesForConfig = dbConnector.getConnection().prepareStatement(
					"SELECT" + " EDSources.superId " + "FROM EDSources " + "JOIN ConfigurationEDSourceAssoc "
			...
			psSelectESSourcesForConfig = dbConnector.getConnection().prepareStatement(
					"SELECT" + " ESSources.superId " + "FROM ESSources " + "JOIN ConfigurationESSourceAssoc "
			...
			psSelectESModulesForConfig = dbConnector.getConnection().prepareStatement(
					"SELECT" + " ESModules.superId " + "FROM ESModules " + "JOIN ConfigurationESModuleAssoc "
			...
			psSelectServicesForConfig = dbConnector.getConnection().prepareStatement(
					"SELECT" + " Services.superId " + "FROM Services " + "JOIN ConfigurationServiceAssoc "
			...
			psSelectSequencesForConfig = dbConnector.getConnection().prepareStatement(
					"SELECT" + " Sequences.sequenceId " + "FROM Sequences " + "JOIN ConfigurationSequenceAssoc "
			...
			psSelectPathsForConfig = dbConnector.getConnection()
					.prepareStatement("SELECT" + " Paths.pathId " + "FROM Paths " + "JOIN ConfigurationPathAssoc "
							+ "ON ConfigurationPathAssoc.pathId=Paths.pathId "
							+ "WHERE ConfigurationPathAssoc.configId=?");
			preparedStatements.add(psSelectPathsForConfig);

			psSelectModulesForSeq = dbConnector.getConnection().prepareStatement(
					"SELECT " + " SequenceModuleAssoc.moduleId " + "FROM SequenceModuleAssoc " + "WHERE sequenceId=?");
			preparedStatements.add(psSelectModulesForSeq);

			psSelectModulesForPath = dbConnector.getConnection().prepareStatement(
					"SELECT " + " PathModuleAssoc.moduleId " + "FROM PathModuleAssoc " + "WHERE pathId=?");
			preparedStatements.add(psSelectModulesForPath);

			psSelectEDSourceTemplatesForRelease = dbConnector.getConnection()
					.prepareStatement("SELECT" + " EDSourceTemplates.superId " + "FROM EDSourceTemplates "
							+ "JOIN SuperIdReleaseAssoc " + "ON SuperIdReleaseAssoc.superId=EDSourceTemplates.superId "
			...
			psSelectESSourceTemplatesForRelease = dbConnector.getConnection()
					.prepareStatement("SELECT" + " ESSourceTemplates.superId " + "FROM ESSourceTemplates "
							+ "JOIN SuperIdReleaseAssoc " + "ON SuperIdReleaseAssoc.superId=ESSourceTemplates.superId "
							+ "WHERE SuperIdReleaseAssoc.releaseId=?");
			preparedStatements.add(psSelectESSourceTemplatesForRelease);

			psSelectESModuleTemplatesForRelease = dbConnector.getConnection()
					.prepareStatement("SELECT" + " ESModuleTemplates.superId " + "FROM ESModuleTemplates "
							+ "JOIN SuperIdReleaseAssoc " + "ON SuperIdReleaseAssoc.superId=ESModuleTemplates.superId "
							+ "WHERE SuperIdReleaseAssoc.releaseId=?");
			preparedStatements.add(psSelectESModuleTemplatesForRelease);

			psSelectServiceTemplatesForRelease = dbConnector.getConnection()
					.prepareStatement("SELECT" + " ServiceTemplates.superId " + "FROM ServiceTemplates "
							+ "JOIN SuperIdReleaseAssoc " + "ON SuperIdReleaseAssoc.superId=ServiceTemplates.superId "
							+ "WHERE SuperIdReleaseAssoc.releaseId=?");
			preparedStatements.add(psSelectServiceTemplatesForRelease);

			psSelectModuleTemplatesForRelease = dbConnector.getConnection()
					.prepareStatement("SELECT" + " ModuleTemplates.superId " + "FROM ModuleTemplates "
							+ "JOIN SuperIdReleaseAssoc " + "ON SuperIdReleaseAssoc.superId=ModuleTemplates.superId "
							+ "WHERE SuperIdReleaseAssoc.releaseId=?");
			preparedStatements.add(psSelectModuleTemplatesForRelease);

			psSelectParametersForSuperId = dbConnector.getConnection()
					.prepareStatement("SELECT" + " SuperIdParameterAssoc.paramId " + "FROM SuperIdParameterAssoc "
							+ "WHERE SuperIdParameterAssoc.superId=?");
			preparedStatements.add(psSelectParametersForSuperId);

			psSelectPSetsForSuperId = dbConnector.getConnection()
					.prepareStatement("SELECT" + " SuperIdParamSetAssoc.psetId " + "FROM SuperIdParamSetAssoc "
							+ "WHERE SuperIdParamSetAssoc.superId=?");
			preparedStatements.add(psSelectPSetsForSuperId);

			psSelectVPSetsForSuperId = dbConnector.getConnection()
					.prepareStatement("SELECT" + " SuperIdVecParamSetAssoc.vpsetId " + "FROM SuperIdVecParamSetAssoc "
							+ "WHERE SuperIdVecParamSetAssoc.superId=?");
			preparedStatements.add(psSelectVPSetsForSuperId);

			psSelectPSetId = dbConnector.getConnection().prepareStatement("SELECT ConfigurationParamSetAssoc.psetId "
					+ "FROM ConfigurationParamSetAssoc " + "WHERE ConfigurationParamSetAssoc.psetId=?");
			preparedStatements.add(psSelectPSetId);

			psSelectEDSourceId = dbConnector.getConnection()
					.prepareStatement("SELECT" + " ConfigurationEDSourceAssoc.edsourceId "
							+ "FROM ConfigurationEDSourceAssoc " + "WHERE ConfigurationEDSourceAssoc.edsourceId=?");
			preparedStatements.add(psSelectEDSourceId);

			psSelectESSourceId = dbConnector.getConnection()
					.prepareStatement("SELECT" + " ConfigurationESSourceAssoc.essourceId "
							+ "FROM ConfigurationESSourceAssoc " + "WHERE ConfigurationESSourceAssoc.essourceId=?");
			preparedStatements.add(psSelectESSourceId);

			psSelectESModuleId = dbConnector.getConnection()
					.prepareStatement("SELECT" + " ConfigurationESModuleAssoc.esmoduleId "
							+ "FROM ConfigurationESModuleAssoc " + "WHERE ConfigurationESModuleAssoc.esmoduleId=?");
			preparedStatements.add(psSelectESModuleId);

			psSelectServiceId = dbConnector.getConnection()
					.prepareStatement("SELECT" + " ConfigurationServiceAssoc.serviceId "
							+ "FROM ConfigurationServiceAssoc " + "WHERE ConfigurationServiceAssoc.serviceId=?");
			...
			psSelectPathId = dbConnector.getConnection().prepareStatement("SELECT" + " ConfigurationPathAssoc.pathId "
					+ "FROM ConfigurationPathAssoc " + "WHERE ConfigurationPathAssoc.pathId=?");
			preparedStatements.add(psSelectPathId);

			psSelectStreamByEventContent = dbConnector.getConnection().prepareStatement(
					"SELECT  ECS.STREAMID " + "FROM    ECSTREAMASSOC   ECS " + "WHERE   ECS.EVENTCONTENTID  = ?");
			preparedStatements.add(psSelectStreamByEventContent);

			psSelectStreamAssocByStream = dbConnector.getConnection().prepareStatement(
					"SELECT ECS.STREAMID " + "FROM	ECSTREAMASSOC	ECS " + "WHERE	ECS.STREAMID = ?");
			preparedStatements.add(psSelectStreamAssocByStream);

			psSelectECStatementByEventContent = dbConnector.getConnection()
					.prepareStatement("SELECT ECSTAT.STATEMENTID " + "FROM	ECSTATEMENTASSOC	ECSTAT "
							+ "WHERE	ECSTAT.EVENTCONTENTID = ?");
			preparedStatements.add(psSelectECStatementByEventContent);

			psSelectECStatementByECStatement = dbConnector.getConnection().prepareStatement("SELECT ECSTAT.STATEMENTID "
					+ "FROM	ECSTATMENTASSOC	ECSTAT " + "WHERE	ECSTAT.STATEMENTID = ? ");
			preparedStatements.add(psSelectECStatementByECStatement);

			psSelectModuleIdBySeq = dbConnector.getConnection()
					.prepareStatement("SELECT" + " SequenceModuleAssoc.moduleId " + "FROM SequenceModuleAssoc "
							+ "WHERE SequenceModuleAssoc.moduleId=?");
			preparedStatements.add(psSelectModuleIdBySeq);

			psSelectModuleIdByPath = dbConnector.getConnection().prepareStatement("SELECT"
					+ " PathModuleAssoc.moduleId " + "FROM PathModuleAssoc " + "WHERE PathModuleAssoc.moduleId=?");
			preparedStatements.add(psSelectModuleIdByPath);

			psSelectTemplateId = dbConnector.getConnection().prepareStatement("SELECT" + " SuperIdReleaseAssoc.superId "
					+ "FROM SuperIdReleaseAssoc " + "WHERE SuperIdReleaseAssoc.superId=?");
			preparedStatements.add(psSelectTemplateId);

			// Event Content, Streams and Primary Datsets

			psSelectEventContentEntries = dbConnector.getConnection()
					.prepareStatement("SELECT ConfigurationContentAssoc.eventContentId,  EventContents.Name "
			...
			psSelectStreamEntries = dbConnector.getConnection()
					.prepareStatement("SELECT streams.streamid, Streams.streamLabel,Streams.fracToDisk, "
							...
							+ "EventContents.eventContentId = " + "ConfigurationContentAssoc.eventContentId WHERE "
							+ "ConfigurationContentAssoc.CONFIGID = ? ");
			psSelectStreamEntries.setFetchSize(1024);
			preparedStatements.add(psSelectStreamEntries);

			psSelectDatasetEntries = dbConnector.getConnection()
					.prepareStatement("SELECT PrimaryDatasets.datasetId, PrimaryDatasets.datasetLabel,"
							+ "Streams.streamid, Streams.streamLabel FROM PrimaryDatasets "
							+ "JOIN StreamDatasetAssoc ON "
							+ "PrimaryDatasets.datasetId = StreamDatasetAssoc.datasetId " + "JOIN Streams ON "
							+ "Streams.streamId = StreamDatasetAssoc.StreamId " + "JOIN ECStreamAssoc ON "
							+ "ECStreamAssoc.StreamId = Streams.streamId " + "JOIN EventContents ON "
			...
			psSelectPathStreamDatasetEntries = dbConnector.getConnection()
					.prepareStatement("SELECT PathStreamDatasetAssoc.pathId, PathStreamDatasetAssoc.streamId,"
							+ "PathStreamDatasetAssoc.datasetId FROM PathStreamDatasetAssoc " + "JOIN Streams ON "
							+ "Streams.streamId = PathStreamDatasetAssoc.StreamId " + "JOIN ECStreamAssoc ON "
							+ "ECStreamAssoc.StreamId = Streams.streamId " + "JOIN EventContents ON "
							+ "EventContents.eventContentId = ECStreamAssoc.eventContentId "
							+ "JOIN ConfigurationContentAssoc ON "
							+ "EventContents.eventContentId = ConfigurationContentAssoc.eventContentId "
			...
			psSelectStatementId = dbConnector.getConnection()
					.prepareStatement("SELECT statementId from EventContentStatements WHERE classN = ? "
							+ " AND moduleL = ? AND extraN = ? AND processN = ? AND statementType = ? ");
			preparedStatements.add(psSelectStatementId);

			psSelectEventContentStatements = dbConnector.getConnection()
					.prepareStatement("Select EventContentStatements.statementId, "
			...
			psSelectReleaseCount = dbConnector.getConnection()
					.prepareStatement("SELECT COUNT(*) FROM SoftwareReleases");
			preparedStatements.add(psSelectReleaseCount);

			psSelectConfigurationCount = dbConnector.getConnection()
					.prepareStatement("SELECT COUNT(*) FROM Configurations");
			preparedStatements.add(psSelectConfigurationCount);

			psSelectDirectoryCount = dbConnector.getConnection().prepareStatement("SELECT COUNT(*) FROM Directories");
			preparedStatements.add(psSelectDirectoryCount);

			psSelectSuperIdCount = dbConnector.getConnection().prepareStatement("SELECT COUNT(*) FROM SuperIds");
			preparedStatements.add(psSelectSuperIdCount);

			psSelectEDSourceTemplateCount = dbConnector.getConnection()
					.prepareStatement("SELECT COUNT(*) FROM EDSourceTemplates");
			preparedStatements.add(psSelectEDSourceTemplateCount);

			psSelectEDSourceCount = dbConnector.getConnection().prepareStatement("SELECT COUNT(*) FROM EDSources");
			preparedStatements.add(psSelectEDSourceCount);

			psSelectESSourceTemplateCount = dbConnector.getConnection()
					.prepareStatement("SELECT COUNT(*) FROM ESSourceTemplates");
			preparedStatements.add(psSelectESSourceTemplateCount);

			psSelectESSourceCount = dbConnector.getConnection().prepareStatement("SELECT COUNT(*) FROM ESSources");
			preparedStatements.add(psSelectESSourceCount);

			psSelectESModuleTemplateCount = dbConnector.getConnection()
					.prepareStatement("SELECT COUNT(*) FROM ESModuleTemplates");
			preparedStatements.add(psSelectESModuleTemplateCount);

			psSelectESModuleCount = dbConnector.getConnection().prepareStatement("SELECT COUNT(*) FROM ESModules");
			preparedStatements.add(psSelectESModuleCount);

			psSelectServiceTemplateCount = dbConnector.getConnection()
					.prepareStatement("SELECT COUNT(*) FROM ServiceTemplates");
			preparedStatements.add(psSelectServiceTemplateCount);

			psSelectServiceCount = dbConnector.getConnection().prepareStatement("SELECT COUNT(*) FROM Services");
			preparedStatements.add(psSelectServiceCount);

			psSelectModuleTemplateCount = dbConnector.getConnection()
					.prepareStatement("SELECT COUNT(*) FROM ModuleTemplates");
			preparedStatements.add(psSelectModuleTemplateCount);

			psSelectModuleCount = dbConnector.getConnection().prepareStatement("SELECT COUNT(*) FROM Modules");
			preparedStatements.add(psSelectModuleCount);

			psSelectSequenceCount = dbConnector.getConnection().prepareStatement("SELECT COUNT(*) FROM Sequences");
			preparedStatements.add(psSelectSequenceCount);

			psSelectPathCount = dbConnector.getConnection().prepareStatement("SELECT COUNT(*) FROM Paths");
			preparedStatements.add(psSelectPathCount);

			psSelectParameterCount = dbConnector.getConnection().prepareStatement("SELECT COUNT(*) FROM Parameters");
			preparedStatements.add(psSelectParameterCount);

			psSelectParameterSetCount = dbConnector.getConnection()
					.prepareStatement("SELECT COUNT(*) FROM ParameterSets");
			preparedStatements.add(psSelectParameterSetCount);

			psSelectVecParameterSetCount = dbConnector.getConnection()
					.prepareStatement("SELECT COUNT(*) FROM VecParameterSets");
			...
			psInsertDirectory = dbConnector.getConnection().prepareStatement(
					"INSERT INTO Directories " + "(parentDirId,dirName,created) " + "VALUES (?, ?, SYSDATE)",
					keyColumn);
			preparedStatements.add(psInsertDirectory);

			if (dbType.equals(dbTypeMySQL))
				psInsertConfiguration = dbConnector.getConnection().prepareStatement(
			...
			psInsertConfigurationLock = dbConnector.getConnection().prepareStatement(
					"INSERT INTO LockedConfigurations (parentDirId,config,userName)" + "VALUES(?, ?, ?)");
			preparedStatements.add(psInsertConfigurationLock);

			// Insert Event Content
			psInsertContents = dbConnector.getConnection()
					.prepareStatement("INSERT INTO EventContents (name)" + "VALUES(?)", keyColumn);
			preparedStatements.add(psInsertContents);

			psInsertContentsConfigAssoc = dbConnector.getConnection().prepareStatement(
					"INSERT INTO ConfigurationContentAssoc (eventContentId,configId)" + "VALUES(?,?)");
			preparedStatements.add(psInsertContentsConfigAssoc);

			psInsertEventContentStatements = dbConnector.getConnection().prepareStatement(
			...
			psInsertStreams = dbConnector.getConnection().prepareStatement(
					"INSERT INTO Streams (streamId,streamLabel,fracToDisk)" + "VALUES(?,?,?)", keyColumn);
			preparedStatements.add(psInsertStreams);

			psInsertPrimaryDatasets = dbConnector.getConnection()
					.prepareStatement("INSERT INTO PrimaryDatasets (datasetLabel)" + "VALUES(?)", keyColumn);
			preparedStatements.add(psInsertPrimaryDatasets);

			psInsertECStreamAssoc = dbConnector.getConnection()
					.prepareStatement("INSERT INTO ECStreamAssoc (eventContentId, streamId)" + "VALUES(?,?)");
			preparedStatements.add(psInsertECStreamAssoc);

			psInsertPathStreamPDAssoc = dbConnector.getConnection().prepareStatement(
					"INSERT INTO PathStreamDatasetAssoc (pathId, streamId, datasetId)" + "VALUES(?,?,?)");
			preparedStatements.add(psInsertPathStreamPDAssoc);

			psInsertECStatementAssoc = dbConnector.getConnection()
					.prepareStatement("INSERT INTO ECStatementAssoc (statementRank,statementId,eventContentId,pathId) "
							+ "VALUES(?,?,?,?) ");
			preparedStatements.add(psInsertECStatementAssoc);

			psInsertStreamDatasetAssoc = dbConnector.getConnection()
					.prepareStatement("INSERT INTO StreamDatasetAssoc (streamId, datasetId)" + "VALUES(?,?)");
			preparedStatements.add(psInsertStreamDatasetAssoc);

			if (dbType.equals(dbTypeMySQL))
				psInsertSuperId = dbConnector.getConnection().prepareStatement("INSERT INTO SuperIds VALUES()",
						keyColumn);
			else if (dbType.equals(dbTypeOracle))
				psInsertSuperId = dbConnector.getConnection().prepareStatement("INSERT INTO SuperIds VALUES('')",
						keyColumn);
			...
			psInsertGlobalPSet = dbConnector.getConnection().prepareStatement(
					"INSERT INTO ConfigurationParamSetAssoc " + "(configId,psetId,sequenceNb) " + "VALUES(?, ?, ?)");
			preparedStatements.add(psInsertGlobalPSet);

			psInsertEDSource = dbConnector.getConnection()
					.prepareStatement("INSERT INTO EDSources (superId,templateId) " + "VALUES(?, ?)");
			preparedStatements.add(psInsertEDSource);

			psInsertConfigEDSourceAssoc = dbConnector.getConnection().prepareStatement("INSERT INTO "
					+ "ConfigurationEDSourceAssoc (configId,edsourceId,sequenceNb) " + "VALUES(?, ?, ?)");
			preparedStatements.add(psInsertConfigEDSourceAssoc);

			psInsertESSource = dbConnector.getConnection()
					.prepareStatement("INSERT INTO " + "ESSources (superId,templateId,name) " + "VALUES(?, ?, ?)");
			preparedStatements.add(psInsertESSource);

			psInsertConfigESSourceAssoc = dbConnector.getConnection()
					.prepareStatement("INSERT INTO " + "ConfigurationESSourceAssoc "
							+ "(configId,essourceId,sequenceNb,prefer) " + "VALUES(?, ?, ?, ?)");
			preparedStatements.add(psInsertConfigESSourceAssoc);

			psInsertESModule = dbConnector.getConnection()
					.prepareStatement("INSERT INTO " + "ESModules (superId,templateId,name) " + "VALUES(?, ?, ?)");
			preparedStatements.add(psInsertESModule);

			psInsertConfigESModuleAssoc = dbConnector.getConnection()
					.prepareStatement("INSERT INTO " + "ConfigurationESModuleAssoc "
							+ "(configId,esmoduleId,sequenceNb,prefer) " + "VALUES(?, ?, ?, ?)");
			preparedStatements.add(psInsertConfigESModuleAssoc);

			psInsertService = dbConnector.getConnection()
					.prepareStatement("INSERT INTO " + "Services (superId,templateId) " + "VALUES(?, ?)");
			preparedStatements.add(psInsertService);

			psInsertConfigServiceAssoc = dbConnector.getConnection().prepareStatement(
					"INSERT INTO " + "ConfigurationServiceAssoc (configId,serviceId,sequenceNb) " + "VALUES(?, ?, ?)");
			preparedStatements.add(psInsertConfigServiceAssoc);

			psInsertPath = dbConnector.getConnection()
					.prepareStatement("INSERT INTO Paths (name,isEndPath) " + "VALUES(?, ?)", keyColumn);
			preparedStatements.add(psInsertPath);

			psInsertConfigPathAssoc = dbConnector.getConnection().prepareStatement(
					"INSERT INTO " + "ConfigurationPathAssoc (configId,pathId,sequenceNb) " + "VALUES(?, ?, ?)");
			preparedStatements.add(psInsertConfigPathAssoc);

			psInsertSequence = dbConnector.getConnection()
					.prepareStatement("INSERT INTO Sequences (name) " + "VALUES(?)", keyColumn);
			...
			psInsertConfigSequenceAssoc = dbConnector.getConnection().prepareStatement("INSERT INTO "
					+ "ConfigurationSequenceAssoc (configId,sequenceId,sequenceNb) " + "VALUES(?, ?, ?)");
			preparedStatements.add(psInsertConfigSequenceAssoc);

			psInsertModule = dbConnector.getConnection()
					.prepareStatement("INSERT INTO Modules (superId,templateId,name) " + "VALUES(?, ?, ?)");
			preparedStatements.add(psInsertModule);

			psInsertSequenceModuleAssoc = dbConnector.getConnection()
					.prepareStatement("INSERT INTO SequenceModuleAssoc (sequenceId,moduleId,sequenceNb,operator) "
							+ "VALUES(?, ?, ?, ?)");
			preparedStatements.add(psInsertSequenceModuleAssoc);

			psInsertSequenceOutputModuleAssoc = dbConnector.getConnection().prepareStatement(
			...
			psInsertPathPathAssoc = dbConnector.getConnection()
					.prepareStatement("INSERT INTO PathInPathAssoc(parentPathId,childPathId,sequenceNb,operator) "
							+ "VALUES(?, ?, ?, ?)");
			preparedStatements.add(psInsertPathPathAssoc);

			psInsertPathSequenceAssoc = dbConnector.getConnection().prepareStatement(
					"INSERT INTO PathSequenceAssoc (pathId,sequenceId,sequenceNb,operator) " + "VALUES(?, ?, ?, ?)");
			preparedStatements.add(psInsertPathSequenceAssoc);

			psInsertSequenceSequenceAssoc = dbConnector.getConnection()
					.prepareStatement("INSERT INTO SequenceInSequenceAssoc"
							+ "(parentSequenceId,childSequenceId,sequenceNb,operator) " + "VALUES(?, ?, ?, ?)");
			preparedStatements.add(psInsertSequenceSequenceAssoc);

			psInsertPathModuleAssoc = dbConnector.getConnection().prepareStatement(
					"INSERT INTO PathModuleAssoc (pathId,moduleId,sequenceNb,operator) " + "VALUES(?, ?, ?, ?)");
			preparedStatements.add(psInsertPathModuleAssoc);

			psInsertPathOutputModuleAssoc = dbConnector.getConnection()
					.prepareStatement("INSERT INTO PathOutputModAssoc (pathId,outputModuleId,sequenceNb,operator) "
							+ "VALUES(?, ?, ?, ?)");
			preparedStatements.add(psInsertPathOutputModuleAssoc);

			psInsertSuperIdReleaseAssoc = dbConnector.getConnection()
					.prepareStatement("INSERT INTO SuperIdReleaseAssoc (superId,releaseId) " + "VALUES(?, ?)");
			preparedStatements.add(psInsertSuperIdReleaseAssoc);

			psInsertServiceTemplate = dbConnector.getConnection()
					.prepareStatement("INSERT INTO ServiceTemplates (superId,name,cvstag) " + "VALUES (?, ?, ?)");
			preparedStatements.add(psInsertServiceTemplate);

			psInsertEDSourceTemplate = dbConnector.getConnection()
					.prepareStatement("INSERT INTO EDSourceTemplates (superId,name,cvstag) " + "VALUES (?, ?, ?)");
			preparedStatements.add(psInsertEDSourceTemplate);

			psInsertESSourceTemplate = dbConnector.getConnection()
					.prepareStatement("INSERT INTO ESSourceTemplates (superId,name,cvstag) " + "VALUES (?, ?, ?)");
			preparedStatements.add(psInsertESSourceTemplate);

			psInsertESModuleTemplate = dbConnector.getConnection()
					.prepareStatement("INSERT INTO ESModuleTemplates (superId,name,cvstag) " + "VALUES (?, ?, ?)");
			preparedStatements.add(psInsertESModuleTemplate);

			psInsertModuleTemplate = dbConnector.getConnection().prepareStatement(
					"INSERT INTO ModuleTemplates (superId,typeId,name,cvstag) " + "VALUES (?, ?, ?, ?)");
			...
			psInsertParameter = dbConnector.getConnection().prepareStatement(
					"INSERT INTO Parameters (paramTypeId,name,tracked) " + "VALUES(?, ?, ?)", keyColumn);
			...
			psInsertSuperIdParamAssoc = dbConnector.getConnection().prepareStatement(
					"INSERT INTO SuperIdParameterAssoc (superId,paramId,sequenceNb) " + "VALUES(?, ?, ?)");
			preparedStatements.add(psInsertSuperIdParamAssoc);

			psInsertBoolParamValue = dbConnector.getConnection()
					.prepareStatement("INSERT INTO BoolParamValues (paramId,value) " + "VALUES (?, ?)");
			preparedStatements.add(psInsertBoolParamValue);

			psInsertInt32ParamValue = dbConnector.getConnection()
					.prepareStatement("INSERT INTO Int32ParamValues (paramId,value,hex) " + "VALUES (?, ?, ?)");
			preparedStatements.add(psInsertInt32ParamValue);

			psInsertUInt32ParamValue = dbConnector.getConnection()
					.prepareStatement("INSERT INTO UInt32ParamValues (paramId,value,hex) " + "VALUES (?, ?, ?)");
			preparedStatements.add(psInsertUInt32ParamValue);

			psInsertInt64ParamValue = dbConnector.getConnection()
					.prepareStatement("INSERT INTO Int64ParamValues (paramId,value,hex) " + "VALUES (?, ?, ?)");
			preparedStatements.add(psInsertInt64ParamValue);

			psInsertUInt64ParamValue = dbConnector.getConnection()
					.prepareStatement("INSERT INTO UInt64ParamValues (paramId,value,hex) " + "VALUES (?, ?, ?)");
			preparedStatements.add(psInsertUInt64ParamValue);

			psInsertDoubleParamValue = dbConnector.getConnection()
					.prepareStatement("INSERT INTO DoubleParamValues (paramId,value) " + "VALUES (?, ?)");
			preparedStatements.add(psInsertDoubleParamValue);

			psInsertStringParamValue = dbConnector.getConnection()
					.prepareStatement("INSERT INTO StringParamValues (paramId,value) " + "VALUES (?, ?)");
			preparedStatements.add(psInsertStringParamValue);

			psInsertEventIDParamValue = dbConnector.getConnection()
					.prepareStatement("INSERT INTO EventIDParamValues (paramId,value) " + "VALUES (?, ?)");
			preparedStatements.add(psInsertEventIDParamValue);

			psInsertInputTagParamValue = dbConnector.getConnection()
					.prepareStatement("INSERT INTO InputTagParamValues (paramId,value) " + "VALUES (?, ?)");
			preparedStatements.add(psInsertInputTagParamValue);

			psInsertESInputTagParamValue = dbConnector.getConnection()
					.prepareStatement("INSERT INTO ESInputTagParamValues (paramId,value) " + "VALUES (?, ?)");
			preparedStatements.add(psInsertESInputTagParamValue);

			psInsertFileInPathParamValue = dbConnector.getConnection()
					.prepareStatement("INSERT INTO FileInPathParamValues (paramId,value) " + "VALUES (?, ?)");
			preparedStatements.add(psInsertFileInPathParamValue);

			psInsertVInt32ParamValue = dbConnector.getConnection().prepareStatement(
					"INSERT INTO VInt32ParamValues " + "(paramId,sequenceNb,value,hex) " + "VALUES (?, ?, ?, ?)");
			preparedStatements.add(psInsertVInt32ParamValue);

			psInsertVUInt32ParamValue = dbConnector.getConnection().prepareStatement(
					"INSERT INTO VUInt32ParamValues " + "(paramId,sequenceNb,value,hex) " + "VALUES (?, ?, ?, ?)");
			preparedStatements.add(psInsertVUInt32ParamValue);

			psInsertVInt64ParamValue = dbConnector.getConnection().prepareStatement(
					"INSERT INTO VInt64ParamValues " + "(paramId,sequenceNb,value,hex) " + "VALUES (?, ?, ?, ?)");
			preparedStatements.add(psInsertVInt64ParamValue);

			psInsertVUInt64ParamValue = dbConnector.getConnection().prepareStatement(
					"INSERT INTO VUInt64ParamValues " + "(paramId,sequenceNb,value,hex) " + "VALUES (?, ?, ?, ?)");
			preparedStatements.add(psInsertVUInt64ParamValue);

			psInsertVDoubleParamValue = dbConnector.getConnection().prepareStatement(
					"INSERT INTO VDoubleParamValues (paramId,sequenceNb,value) " + "VALUES (?, ?, ?)");
			preparedStatements.add(psInsertVDoubleParamValue);

			psInsertVStringParamValue = dbConnector.getConnection().prepareStatement(
					"INSERT INTO VStringParamValues (paramId,sequenceNb,value) " + "VALUES (?, ?, ?)");
			preparedStatements.add(psInsertVStringParamValue);

			psInsertVEventIDParamValue = dbConnector.getConnection().prepareStatement(
					"INSERT INTO VEventIDParamValues (paramId,sequenceNb,value) " + "VALUES (?, ?, ?)");
			preparedStatements.add(psInsertVEventIDParamValue);

			psInsertVInputTagParamValue = dbConnector.getConnection().prepareStatement(
					"INSERT INTO VInputTagParamValues (paramId,sequenceNb,value) " + "VALUES (?, ?, ?)");
			preparedStatements.add(psInsertVInputTagParamValue);

			psInsertVESInputTagParamValue = dbConnector.getConnection().prepareStatement(
					"INSERT INTO VESInputTagParamValues (paramId,sequenceNb,value) " + "VALUES (?, ?, ?)");
			...
			psDeleteDirectory = dbConnector.getConnection().prepareStatement("DELETE FROM Directories WHERE dirId=?");
			preparedStatements.add(psDeleteDirectory);

			psDeleteLock = dbConnector.getConnection()
					.prepareStatement("DELETE FROM LockedConfigurations " + "WHERE parentDirId=? AND config=?");
			preparedStatements.add(psDeleteLock);

			psDeleteConfiguration = dbConnector.getConnection()
					.prepareStatement("DELETE FROM CONFIGURATIONS " + "WHERE configId = ?");
			preparedStatements.add(psDeleteConfiguration);

			psDeleteSoftwareRelease = dbConnector.getConnection()
					.prepareStatement("DELETE FROM SOFTWARERELEASES " + "WHERE releaseId = ?");
			preparedStatements.add(psDeleteSoftwareRelease);

			psDeletePSetsFromConfig = dbConnector.getConnection()
					.prepareStatement("DELETE FROM ConfigurationParamSetAssoc " + "WHERE configId=?");
			preparedStatements.add(psDeletePSetsFromConfig);

			psDeleteContentFromConfig = dbConnector.getConnection()
					.prepareStatement("DELETE FROM	ConfigurationContentAssoc	" + "WHERE configId = ?");
			preparedStatements.add(psDeleteContentFromConfig);

			psDeleteEDSourcesFromConfig = dbConnector.getConnection()
					.prepareStatement("DELETE FROM ConfigurationEDSourceAssoc " + "WHERE configId=?");
			preparedStatements.add(psDeleteEDSourcesFromConfig);

			psDeleteESSourcesFromConfig = dbConnector.getConnection()
					.prepareStatement("DELETE FROM ConfigurationESSourceAssoc " + "WHERE configId=?");
			preparedStatements.add(psDeleteESSourcesFromConfig);

			psDeleteESModulesFromConfig = dbConnector.getConnection()
					.prepareStatement("DELETE FROM ConfigurationESModuleAssoc " + "WHERE configId=?");
			preparedStatements.add(psDeleteESModulesFromConfig);

			psDeleteServicesFromConfig = dbConnector.getConnection()
					.prepareStatement("DELETE FROM ConfigurationServiceAssoc " + "WHERE configId=?");
			preparedStatements.add(psDeleteServicesFromConfig);

			psDeleteSequencesFromConfig = dbConnector.getConnection()
					.prepareStatement("DELETE FROM ConfigurationSequenceAssoc " + "WHERE configId=?");
			preparedStatements.add(psDeleteSequencesFromConfig);

			psDeletePathsFromConfig = dbConnector.getConnection()
					.prepareStatement("DELETE FROM ConfigurationPathAssoc " + "WHERE configId=?");
			preparedStatements.add(psDeletePathsFromConfig);

			/*
			 * 28/09/2009 psDeleteStreamsAndDatasetsFromConfig =
			 * dbConnector.getConnection().prepareStatement
			 * ("DELETE FROM ConfigurationStreamAssoc "+ "WHERE configId=?");
			 * preparedStatements.add(psDeleteStreamsAndDatasetsFromConfig);
			 */

			psDeleteChildSeqsFromParentSeq = dbConnector.getConnection()
					.prepareStatement("DELETE FROM SequenceInSequenceAssoc " + "WHERE parentSequenceId=?");
			preparedStatements.add(psDeleteChildSeqsFromParentSeq);

			psDeleteChildSeqFromParentSeqs = dbConnector.getConnection()
					.prepareStatement("DELETE FROM SequenceInSequenceAssoc " + "WHERE childSequenceId=?");
			preparedStatements.add(psDeleteChildSeqFromParentSeqs);

			psDeleteChildSeqsFromParentPath = dbConnector.getConnection()
					.prepareStatement("DELETE FROM PathSequenceAssoc " + "WHERE pathId=?");
			preparedStatements.add(psDeleteChildSeqsFromParentPath);

			psDeleteChildSeqFromParentPaths = dbConnector.getConnection()
					.prepareStatement("DELETE FROM PathSequenceAssoc " + "WHERE sequenceId=?");
			preparedStatements.add(psDeleteChildSeqFromParentPaths);

			psDeletePathStreamDataSetAssoc = dbConnector.getConnection()
					.prepareStatement("DELETE FROM	PathStreamDataSetAssoc " + "WHERE			pathId = ?");
			preparedStatements.add(psDeletePathStreamDataSetAssoc);

			psDeletePathOutputModAssoc = dbConnector.getConnection()
					.prepareStatement("DELETE FROM 	PathOutputModAssoc " + "WHERE			pathId = ?");
			preparedStatements.add(psDeletePathOutputModAssoc);

			psDeleteECStreamFromEventCont = dbConnector.getConnection()
					.prepareStatement("DELETE FROM	ECStreamAssoc " + "WHERE			eventContentId = ?");
			preparedStatements.add(psDeleteECStreamFromEventCont);

			psDeleteECStatementFromEventCont = dbConnector.getConnection()
					.prepareStatement("DELETE FROM	ECStatementAssoc " + "WHERE			eventContentId = ?");

			psDeleteEventContentStatement = dbConnector.getConnection()
					.prepareStatement("DELETE FROM 	eventcontentstatements " + "WHERE			statementid = ?");

			psDeleteChildPathsFromParentPath = dbConnector.getConnection()
					.prepareStatement("DELETE FROM PathInPathAssoc " + "WHERE parentPathId=?");
			preparedStatements.add(psDeleteChildPathsFromParentPath);

			psDeleteChildPathFromParentPaths = dbConnector.getConnection()
					.prepareStatement("DELETE FROM PathInPathAssoc " + "WHERE childPathId=?");
			preparedStatements.add(psDeleteChildPathFromParentPaths);

			psDeleteModulesFromSeq = dbConnector.getConnection()
					.prepareStatement("DELETE FROM SequenceModuleAssoc " + "WHERE sequenceId=?");
			preparedStatements.add(psDeleteModulesFromSeq);

			psDeleteModulesFromPath = dbConnector.getConnection()
					.prepareStatement("DELETE FROM PathModuleAssoc " + "WHERE pathId=?");
			preparedStatements.add(psDeleteModulesFromPath);

			psDeleteTemplateFromRelease = dbConnector.getConnection()
					.prepareStatement("DELETE FROM SuperIdReleaseAssoc " + "WHERE superId=? AND releaseId=?");
			preparedStatements.add(psDeleteTemplateFromRelease);

			psDeleteParametersForSuperId = dbConnector.getConnection()
					.prepareStatement("DELETE FROM SuperIdParameterAssoc " + "WHERE superId=?");
			preparedStatements.add(psDeleteParametersForSuperId);

			psDeletePSetsForSuperId = dbConnector.getConnection()
					.prepareStatement("DELETE FROM SuperIdParamSetAssoc " + "WHERE superId=?");
			preparedStatements.add(psDeletePSetsForSuperId);

			psDeleteVPSetsForSuperId = dbConnector.getConnection()
					.prepareStatement("DELETE FROM SuperIdVecParamSetAssoc " + "WHERE superId=?");
			preparedStatements.add(psDeleteVPSetsForSuperId);

			psDeleteSuperId = dbConnector.getConnection().prepareStatement("DELETE FROM SuperIds WHERE superId=?");
			preparedStatements.add(psDeleteSuperId);

			psDeleteParameter = dbConnector.getConnection()
					.prepareStatement("DELETE FROM Parameters WHERE paramId = ?");
			preparedStatements.add(psDeleteParameter);

			psDeletePSet = dbConnector.getConnection().prepareStatement("DELETE FROM ParameterSets WHERE superId = ?");
			preparedStatements.add(psDeletePSet);

			psDeleteVPSet = dbConnector.getConnection()
					.prepareStatement("DELETE FROM VecParameterSets WHERE superId = ?");
			preparedStatements.add(psDeleteVPSet);

			psDeleteSequence = dbConnector.getConnection()
					.prepareStatement("DELETE FROM Sequences WHERE sequenceId = ?");
			preparedStatements.add(psDeleteSequence);

			psDeletePath = dbConnector.getConnection().prepareStatement("DELETE FROM Paths WHERE pathId = ?");
			...
			//
			psSelectTemplates = dbConnector.getConnection()
					.prepareStatement("SELECT" + " template_id," + " template_type," + " template_name,"
							+ " template_cvstag," + " template_pkgid " + "FROM tmp_template_table");
			psSelectTemplates.setFetchSize(1024);
			preparedStatements.add(psSelectTemplates);

			psSelectInstances = dbConnector.getConnection()
					.prepareStatement("SELECT DISTINCT" + " instance_id," + " template_id," + " instance_type,"
							+ " instance_name," + " flag," + " sequence_nb " + "FROM tmp_instance_table "
							+ "ORDER BY instance_type,sequence_nb");
			psSelectInstances.setFetchSize(1024);
			preparedStatements.add(psSelectInstances);

			psSelectParameters = dbConnector.getConnection()
					.prepareStatement("SELECT DISTINCT" + " parameter_id," + " parameter_type," + " parameter_name,"
							+ " parameter_trkd," + " parameter_seqnb," + " parent_id " + "FROM tmp_parameter_table");
			...
			/**
			 * Query new CLOB fields in temp_string_table. NOTE: the rowid JOIN is to
			 * simulate the DISTINCT modifier in the original query, which might be a bad
			 * design. I have managed to patch it up and group the results as the DISTINCT
			 * does since it is not possible to apply DISTINCT and/or GROUPBY in CLOB
			 * queries. psSelectCLOBsValues = dbConnector.getConnection().prepareStatement
			 * ("SELECT " + " clobs.parameter_id , " + " clobs.parameter_value , " + "
			 * clobs.sequence_nb " + "FROM " + " ( " + " SELECT Max(rowid) idrow , " + "
			 * parameter_id , " + " sequence_nb " + " FROM tmp_string_table " + " GROUP BY
			 * parameter_id , " + " sequence_nb " + " ) grouped " + "JOIN " + "
			 * tmp_string_table clobs " + "ON (clobs.rowid = grouped.idrow)");
			 */
			psSelectCLOBsValues = dbConnector.getConnection().prepareStatement(
					"SELECT 						" + "   parameter_id    ,	" + "   parameter_value ,	"
							+ "   sequence_nb 		" + "FROM					" + "   tmp_clob_table 	");
			psSelectCLOBsValues.setFetchSize(2048);
			preparedStatements.add(psSelectCLOBsValues);

			psSelectPathEntries = dbConnector.getConnection()
					.prepareStatement("SELECT" + " path_id," + " entry_id," + " sequence_nb," + " entry_type, "
							+ " operator " + "FROM tmp_path_entries " + "ORDER BY path_id ASC, sequence_nb ASC");
			...
			psSelectSequenceEntries = dbConnector.getConnection().prepareStatement(
					"SELECT" + " sequence_id," + " entry_id," + " sequence_nb," + " entry_type " + " operator "
							+ "FROM tmp_sequence_entries " + "ORDER BY sequence_id ASC, sequence_nb ASC");
			psSelectSequenceEntries.setFetchSize(1024);
			preparedStatements.add(psSelectSequenceEntries);

			// bug #91797 ConfDB operator IGNORE/NEGATE also for modules in a sequence
			psSelectSequenceEntriesAndOperator = dbConnector.getConnection().prepareStatement(
					"SELECT" + " sequence_id," + " entry_id," + " sequence_nb," + " entry_type, " + " operator "
							+ "FROM tmp_sequence_entries " + "ORDER BY sequence_id ASC, sequence_nb ASC");
			...
			psInsertReleaseTag = dbConnector.getConnection()
					.prepareStatement("INSERT INTO SoftwareReleases " + " (releaseTag)  VALUES (?)", keyColumn);
			...
			psSelectSoftwareSubsystemId = dbConnector.getConnection()
					.prepareStatement("SELECT subsysId " + "FROM SoftwareSubsystems " + "WHERE name = ?");
			...
			psInsertSoftwareSubsystem = dbConnector.getConnection()
					.prepareStatement("INSERT INTO SoftwareSubsystems " + " (name) VALUES (?)", keyColumn);
			preparedStatements.add(psInsertSoftwareSubsystem);

			psSelectSoftwarePackageId = dbConnector.getConnection().prepareStatement(
					"SELECT packageId " + "FROM SoftwarePackages " + "WHERE subsysId = ? AND name = ? ");
			...
			psInsertSoftwarePackage = dbConnector.getConnection()
					.prepareStatement("INSERT INTO SoftwarePackages " + " (subsysId, name) VALUES (?,?)", keyColumn);
			preparedStatements.add(psInsertSoftwarePackage);

			psInsertEDSourceTemplateRelease = dbConnector.getConnection().prepareStatement(
					"INSERT INTO EDSourceTemplates " + " (superId, name, CVSTAG,packageId) VALUES (?,?,?,?)");
			...
			psInsertESSourceTemplateRelease = dbConnector.getConnection().prepareStatement(
					"INSERT INTO ESSourceTemplates " + " (superId, name, CVSTAG,packageId) VALUES (?,?,?,?)");
			...
			psInsertESModuleTemplateRelease = dbConnector.getConnection().prepareStatement(
					"INSERT INTO ESModuleTemplates " + " (superId, name, CVSTAG,packageId) VALUES (?,?,?,?)");
			...
			psInsertServiceTemplateRelease = dbConnector.getConnection().prepareStatement(
					"INSERT INTO ServiceTemplates " + " (superId, name, CVSTAG,packageId) VALUES (?,?,?,?)");
			...
			psInsertModuleTemplateRelease = dbConnector.getConnection().prepareStatement(
					"INSERT INTO ModuleTemplates " + " (superId, typeId, name, CVSTAG,packageId) VALUES (?,?,?,?,?)");
			...
			psCheckPathFieldsExistence = dbConnector.getConnection()
					.prepareStatement("Select 1 from dual where exists (			"
							+ "select 1 from all_tab_columns			" + "where table_name = 'PATHS'				"
							+ "and column_name = 'DESCRIPTION')			");
			preparedStatements.add(psCheckPathFieldsExistence);

			psSelectPathExtraFields = dbConnector.getConnection()
					.prepareStatement("SELECT p.pathid		,	" + "		p.description	, 	"
							+ "		p.contact			" + "FROM	Paths	p			"
							+ "WHERE	p.pathid = ?		");
			preparedStatements.add(psSelectPathExtraFields);

			psInsertPathDescription = dbConnector.getConnection().prepareStatement(
					"INSERT INTO Paths (name,isEndPath, description, contact) " + "VALUES(?, ?, ?, ?)", keyColumn);
			preparedStatements.add(psInsertPathDescription);

			// bug #91797: ConfDB operator IGNORE/NEGATE also for modules in a sequence.
			psCheckOperatorForModuleSequenceAssoc = dbConnector.getConnection()
					.prepareStatement("select 1 from dual where exists ( 		"
							+ " select 1 from all_tab_columns 			" + " where table_name = 'TMP_SEQUENCE_ENTRIES'"
	...
	private HashMap<Integer, ArrayList<Parameter>> getParameters() throws DatabaseException {
		...
		try {
			...
			while (rsBooleanValues.next()) {
				...
				String valueAsString = (new Boolean(rsBooleanValues.getBoolean(2))).toString();
			...
			while (rsIntValues.next()) {
				...
				String valueAsString = (isHex) ? "0x" + Long.toHexString(value) : Long.toString(value);

				if (sequenceNb != null && idToValueAsString.containsKey(parameterId))
					idToValueAsString.put(parameterId, idToValueAsString.get(parameterId) + ", " + valueAsString);
				else
					idToValueAsString.put(parameterId, valueAsString);
			}
			...
			while (rsRealValues.next()) {
				...
				String valueAsString = (new Double(rsRealValues.getDouble(2))).toString();
	...
	private void insertVecParameterSet(int superId, int sequenceNb, VPSetParameter vpset) throws DatabaseException {
		...
		try {
			...
			String errMsg = "ConfDBV1::insertVecParameterSet(superId=" + superId + ",sequenceNb=" + sequenceNb
					+ ",vpset=" + vpset.name() + ") failed: " + e.getMessage();
			throw new DatabaseException(errMsg, e);
		} finally {
			dbConnector.release(rs);
		}
	...
	private void insertParameterSet(int superId, int sequenceNb, PSetParameter pset) throws DatabaseException {
		int psetId = insertSuperId();
		ResultSet rs = null;
		try {
			psInsertParameterSet.setInt(1, psetId);
			...
			String errMsg = "ConfDBV1::insertParameterSet(superId=" + superId + ",sequenceNb=" + sequenceNb + ",pset="
	...
	private void insertParameter(int superId, int sequenceNb, Parameter parameter) throws DatabaseException {
		int paramId = 0;
		ResultSet rs = null;
		try {
			...
			String errMsg = "ConfDBV1::insertParameter(superId=" + superId + ",sequenceNb=" + sequenceNb + ",parameter="
	...
	private void insertSuperIdParamAssoc(int superId, int paramId, int sequenceNb) throws DatabaseException {
		try {
			...
			String errMsg = "ConfDBV1::insertSuperIdParamAssoc(superId=" + superId + ",paramId=" + paramId
	...
	private void insertSuperIdParamSetAssoc(int superId, int psetId, int sequenceNb) throws DatabaseException {
		try {
			...
			String errMsg = "ConfDBV1::inesrtSuperIdParamSetAssoc(superId=" + superId + ",psetId=" + psetId
					+ ",sequenceNb=" + sequenceNb + ") failed: " + e.getMessage();
			throw new DatabaseException(errMsg, e);
		}
	}
	...
	private void insertSuperIdVecParamSetAssoc(int superId, int vpsetId, int sequenceNb) throws DatabaseException {
		try {
			...
			String errMsg = "ConfDBV1::inesrtSuperIdVecParamSetAssoc(superId=" + superId + ",vpsetId=" + vpsetId
					+ ",sequenceNb=" + sequenceNb + ") failed: " + e.getMessage();
			throw new DatabaseException(errMsg, e);
		}
	}
	...
	private void insertParameterValue(int paramId, Parameter parameter) throws DatabaseException {
		...
		try {
			...
			String errMsg = "ConfDBV1::insertParameterValue(paramId=" + paramId + ",parameter=" + parameter.name()
					+ ") failed: " + e.getMessage();
			throw new DatabaseException(errMsg, e);
		}
	}
	...
	private void insertSuperIdReleaseAssoc(int superId, String releaseTag) throws DatabaseException {
		...
		try {
			...
			String errMsg = "ConfDBV1::insertSuperIdReleaseAssoc(superId=" + superId + ",releaseTag=" + releaseTag
					+ ") failed: " + e.getMessage();
			throw new DatabaseException(errMsg, e);
		}
	}
	...
	private void insertSuperIdReleaseAssoc(int superId, int releaseId) throws DatabaseException {
		try {
			...
			String errMsg = "ConfDBV1::insertSuperIdReleaseAssoc(superId=" + superId + ",releaseId=" + releaseId
					+ ") failed: " + e.getMessage();
			throw new DatabaseException(errMsg, e);
		}
	}
	...
	public int getReleaseId(String releaseTag) throws DatabaseException {
		...
		try {
			...
			String errMsg = "ConfDBV1::getReleaseId(releaseTag=" + releaseTag + ") failed: " + e.getMessage();
			throw new DatabaseException(errMsg, e);
		} finally {
			dbConnector.release(rs);
		}
	...
	public static void main(String[] args) {
		...
		try {
			...
			// configurations
				...
				int id = (configId.length() > 0) ? Integer.parseInt(configId) : database.getConfigId(configName);
				if (id <= 0)
