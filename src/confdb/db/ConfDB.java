...
public class ConfDB
{
    ...
    public synchronized Directory loadConfigurationTree() throws DatabaseException
    {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                //System.err.println("Retrieved Dirs: "+dirId+"- parent "+parentDirId+" "+dirName+" "+dirCreated);
                if (directoryHashMap.size()==0) {
                    //System.err.println("New root DIrectrory: "+dirId+"- "+dirName+" "+dirCreated);
                    ...
                    //System.err.println("New DIrectrory with parent: "+dirId+"- "+dirName+" "+dirCreated+ parentDirId);
            ...
            while (rs.next()) {
                ...
                //System.err.println("Retrieved Conf: "+configName+" "+configCreated+" "+configReleaseTag);
                ...
                //System.err.println("ConfigHash done: "+configName+" "+configCreated+" "+configReleaseTag);
            }
        ...
        throws DatabaseException
    {
        ...
        try {
            //System.err.println("loadTemplate with ReleaseTag: "+releaseId+" "+templateName);
    ...
    {
        int releaseId = getReleaseId(releaseTag);
        //System.err.println("loadSoftwareRelease with releaseTag(rec ID): "+releaseTag+" ("+releaseId+") "+release);
        ...
        try {
           int releaseId = getReleaseId(release.releaseTag());
            ...
            while (rsTemplates.next()) {
                ...
                //System.err.println("loadTemplates: id "+id+" type "+" name "+name+" cvstag "+cvstag+" pkgid "+pkgId);
                //System.err.println("Template "+templateId+" "+templateName+" instance "+instanceName);
        ...
        throws DatabaseException
    {
        ...
        //System.err.println("loadTemplates with configid: "+configId+" release "+release);
        ...
        throws DatabaseException
    {
        ...
        //System.err.println("loadTemplates with configinfo: "+releaseTag+" release "+release);
        ...
        throws DatabaseException
    {
        ...
        try {
            ...
            while (rsInstances.next()) {
                ...
                //System.err.println("found instance "+id+"name="+instanceName+" templateid="+templateId+" entryType="+type);
                if (type.equals("PSet")) {
            ...
            while (rsStreamEntries.next()) {
                ...
                //System.err.println("Stream id "+streamId+" Label "+streamLabel+" fracTo "+ fracToDisk+" Evco id "+eventContentId+" name "+name);
                if(eventContent==null) continue;

                //System.err.println("Adding Stream id "+streamId+" Label "+streamLabel+" fracTo "+ fracToDisk+" Evco id "+eventContentId+" name "+name);
                ...
                while (it.hasNext()) {
                    ...
                    /*if (p.type().equals("vstring"))
                    {
                        ...
                        //System.err.println(" Reread Param "+p2.type()+" name "+p2.name()+" val "+p2.valueAsString()+" size "+p2.vectorSize());
                        if (p2.vectorSize()>0)
                          for (int x=0;x<p2.vectorSize();x++)
                             //System.err.println(" reread values " + x + " " +p2.value(x));
                    }*/
                }
            ...
            while (rsSequenceEntries.next()) {
                ...
                //System.err.println("found seq "+ entryId + "parent="+sequenceId+ " lvl="+entryLvl+" ord "+sequenceNb+" entryType="+entryType);
                ...
                if ((entryLvl>0) && (!seqtoskip))
                {
                    ...
                    if(entry==null) {System.err.println("ERROR: can't find stream for entryid"); continue;}
            ...
            while (rsDatasetEntries.next()) {
              ...
              //System.err.println("Dataset entry label "+datasetLabel+" id "+datasetId+" stream "+streamLabel+" streamid "+streamId);
                if((stream == null) || (datasetLabel.equals("Unassigned path")))
                    continue;

              //System.err.println("Adding dataset "+datasetLabel+" id "+datasetId+" stream "+streamLabel+" streamid "+streamId);
            ...
            while (rsPathStreamDataset.next()) {
                        ...
                        //System.err.println("Path to dataset "+datasetId+" id "+pathId+" streamid "+streamId);
                        ...
                        //System.err.println("Adding Path to dataset "+datasetId+" id "+pathId+" streamid "+streamId);
         ...
         //System.err.println("###########################End of loops###################");
        ...
        throws DatabaseException
    {
        ...
        try {
            for (int sequenceNb=0;sequenceNb<config.pathCount();sequenceNb++) {
                ...
                //System.err.println("insertPath: Trying to insert confver "+configId+" v pathid "+vpathId);
...
/*not allowed anymore               if (r instanceof PathReference) {
         ...
         //System.err.println("insertReferences - Found  seq " + sequenceId + " ( " + r.name() + " )");
                        try {
...
//          int      sequenceId = sequenceHashMap.get(sequence.name());
         ...
         //System.err.println("insertSeqRef - Found " + sequence.entryCount() + " for seq " + sequenceId + " ( " + sequence.name() + " )");
    ...
    public synchronized void insertTemplateIntoRelease(SoftwarePackage softwarePackage,int pkgId,int releaseId) throws SQLException
    {
        ...
        while (templateIt.hasNext()) {
            ...
            if (template instanceof EDSourceTemplate) {
                ...
                System.out.println("ESM name="+template.name()+" cvsTag="+template.cvsTag()+" pkgId="+pkgId);
...
*/
           psInsertEDSourceT2Rele =
                dbConnector.getConnection().prepareStatement
                ("insert into u_edst2rele (id_edstemplate,id_release) values (?,?)");
            preparedStatements.add(psInsertEDSourceT2Rele);

           psInsertESSourceT2Rele =
                dbConnector.getConnection().prepareStatement
                ("insert into u_esst2rele (id_esstemplate,id_release) values (?,?)");
            preparedStatements.add(psInsertESSourceT2Rele);

           psInsertESModuleT2Rele =
                dbConnector.getConnection().prepareStatement
                ("insert into u_esmt2rele (id_esmtemplate,id_release) values (?,?)");
            preparedStatements.add(psInsertESModuleT2Rele);

           psInsertServiceT2Rele =
                dbConnector.getConnection().prepareStatement
                ("insert into u_srvt2rele (id_srvtemplate,id_release) values (?,?)");
            preparedStatements.add(psInsertServiceT2Rele);

           psInsertModuleT2Rele =
                dbConnector.getConnection().prepareStatement
                ("insert into u_modt2rele (id_modtemplate,id_release) values (?,?)");
  ...
  ("INSERT INTO u_esmtemplates (id_pkg,name,cvstag) " +
   "VALUES (?, ?, ?)", keyColumn);
            ...
            psInsertModuleTemplate =
                dbConnector.getConnection().prepareStatement
                ("INSERT INTO u_ModuleTemplates (typeId,name,cvstag) " +
                 "VALUES (?, ?, ?)");
...
*/
  ...
  ("INSERT INTO u_edstemplates "+
   " (name, CVSTAG, id_pkg) VALUES (?,?,?)",keyColumn);
 ...
 */
                ...
                //System.err.println("ParId "+parentId+" (origparid "+orparid+") parameterId "+parameterId+" type "+ type+" name "+name+" seqNb "+seqNb+" lvl"+lvl);
