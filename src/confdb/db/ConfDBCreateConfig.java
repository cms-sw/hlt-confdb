...
public class ConfDBCreateConfig {
	...
	public static void main(String[] args) {
		...
		if (masterConfigName.length() == 0) {
			System.err.println("master config-name must be specified " + "(-m / --master)");
			System.exit(0);
		}
	...
	static public void doIt(String dbType, String dbUrl, String dbUser, String dbPwrd, String masterConfigName,
			String newConfigName, HashSet<String> pathsToInclude, PrintStream out, String userName) throws Exception {
		...
		try {
			...
			String dirName = newConfigName.substring(0, newConfigName.lastIndexOf('/'));
			...
			if (pathsToInclude.size() != 0) {
				...
				while (it.hasNext())
					sberrmsg.append(it.next()).append(" ");
			...
			String comment = "Created by ConfDBCreateConfig " + "from master " + masterConfig + ".";
			...
			out.println("... stored as " + masterConfig + " (" + elapsedTime + " seconds)");
		} finally {
			try {
				database.disconnect();
			} catch (DatabaseException e) {
