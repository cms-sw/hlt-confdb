...
public class SoftwareRelease implements Serializable {
			...
			while (pkgIt.hasNext()) {
				...
				while (templateIt.hasNext()) {
					...
					if (template instanceof EDSourceTemplate) {
						EDSourceTemplate newTemplate = new EDSourceTemplate(template.name(), template.cvsTag(),
								parameters);
						newTemplate.setDatabaseId(template.databaseId());
						pkg.addTemplate(newTemplate);
					} else if (template instanceof ESSourceTemplate) {
						ESSourceTemplate newTemplate = new ESSourceTemplate(template.name(), template.cvsTag(),
								parameters);
						newTemplate.setDatabaseId(template.databaseId());
						pkg.addTemplate(newTemplate);
					} else if (template instanceof ESModuleTemplate) {
						ESModuleTemplate newTemplate = new ESModuleTemplate(template.name(), template.cvsTag(),
								parameters);
						newTemplate.setDatabaseId(template.databaseId());
						pkg.addTemplate(newTemplate);
					} else if (template instanceof ServiceTemplate) {
						ServiceTemplate newTemplate = new ServiceTemplate(template.name(), template.cvsTag(),
								parameters);
						newTemplate.setDatabaseId(template.databaseId());
						pkg.addTemplate(newTemplate);
					} else if (template instanceof ModuleTemplate) {
						ModuleTemplate newTemplate = new ModuleTemplate(template.name(), template.cvsTag(), parameters,
								template.type());
						newTemplate.setDatabaseId(template.databaseId());
						pkg.addTemplate(newTemplate);
					}
	...
	public SoftwareSubsystem subsystem(int i) {
		return subsystems.get(i);
	}

	/** get subsystem iterator */
	public Iterator<SoftwareSubsystem> subsystemIterator() {
		return subsystems.iterator();
	}

	/** index of a certain subsystem */
	public int indexOfSubsystem(SoftwareSubsystem s) {
		return subsystems.indexOf(s);
	}
	...
	public ArrayList<String> listOfReferencedPackages() {
		...
		while (itS.hasNext()) {
			...
			if (subsys.referencedPackageCount() == 0)
				continue;
		...
		while (itS.hasNext()) {
			...
			while (itP.hasNext()) {
				...
				if (pkg.templateCount() > 0)
					result.add(pkg.template(0).cvsTag() + " " + subsys.name() + "/" + pkg.name());
			}
		}
		return result;
	}
	...
	public int edsourceTemplateCount() {
		return edsourceTemplates.size();
	}

	/** number of ESSource templates */
	public int essourceTemplateCount() {
		return essourceTemplates.size();
	}

	/** number of ESModule templates */
	public int esmoduleTemplateCount() {
		return esmoduleTemplates.size();
	}

	/** number of Service templates */
	public int serviceTemplateCount() {
		return serviceTemplates.size();
	}

	/** number of Module templates */
	public int moduleTemplateCount() {
		return moduleTemplates.size();
	}
	...
	private boolean addTemplate(Template template) {
		if (template instanceof EDSourceTemplate) {
			...
			if (!edsourceTemplatesByName.containsKey(edsource.name())) {
				...
				edsourceTemplateNamesByDbId.put(edsource.databaseId(), edsource.name());
			...
			if (!essourceTemplatesByName.containsKey(essource.name())) {
				...
				essourceTemplateNamesByDbId.put(essource.databaseId(), essource.name());
			...
			if (!esmoduleTemplatesByName.containsKey(esmodule.name())) {
				...
				esmoduleTemplateNamesByDbId.put(esmodule.databaseId(), esmodule.name());
			...
			if (!serviceTemplatesByName.containsKey(service.name())) {
				...
				serviceTemplateNamesByDbId.put(service.databaseId(), service.name());
			...
			if (!moduleTemplatesByName.containsKey(module.name())) {
				...
				moduleTemplateNamesByDbId.put(module.databaseId(), module.name());
