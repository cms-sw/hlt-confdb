...
public class ConfigInfo implements Comparable<ConfigInfo>, Serializable {
	...
	public ConfigInfo(String name, Directory parentDir, int dbId, int version, String created, String creator,
			String releaseTag, String processName, String comment) {
		this.name = name;
		this.parentDir = parentDir;
		this.releaseTag = releaseTag;
		this.versionIndex = 0;
		versions.add(new ConfigVersion(dbId, version, created, creator, releaseTag, processName, comment));
	}
	...
	public String fullName() {
		return parentDir().name() + "/" + name() + "/V" + version();
	}
	...
	public ConfigVersion version(int i) {
	...
	public int versionCount(String releaseTag) {
		...
		while (itCV.hasNext()) {
			...
			if (releaseTag.equals("") || releaseTag.equals(version.releaseTag()))
				result++;
		}
		return result;
	}
	...
	public boolean lock(String userName) {
		if (isLocked() || userName.equals(new String()))
			return false;
	...
	public void addVersion(int dbId, int version, String created, String creator, String releaseTag, String processName,
			String comment) {
		...
		ConfigVersion configVersion = new ConfigVersion(dbId, version, created, creator, releaseTag, processName,
				comment);
	...
	public void setVersionIndex(String releaseTag, int index) {
		if (releaseTag.equals("")) {
			...
			while (j < versions.size() && i < index) {
				if (versions.get(j).releaseTag().equals(releaseTag))
					i++;
				j++;
			}
			versionIndex = j - 1;
