...
public class DatabaseMigrator {
	...
	public DatabaseMigrator(Configuration sourceConfig, ConfDB sourceDB, ConfDB targetDB) {
	...
	public void migrate(String targetName, Directory targetDir) throws MigratorException {
		...
		try {
			...
			targetDB.insertConfiguration(targetConfig, creator, sourceConfig.processName(),
					"imported from external database.");
		} catch (DatabaseException e) {
			String errMsg = "DatabaseMigrator::migrate(targetName=" + targetName + ",targetDir=" + targetDir.name()
					+ ") failed.";
	...
	public ReleaseMigrator releaseMigrator() {
		return releaseMigrator;
	}

	/** get source configuration */
	public Configuration sourceConfig() {
		return sourceConfig;
	}
